 ...
 * @am_payment_api 6.0
 */
class Am_Paysystem_Braintree extends Am_Paysystem_CreditCard_Token
{
    const
        PLUGIN_STATUS = self::STATUS_PRODUCTION,
        PLUGIN_DATE = '$Date$',
        PLUGIN_REVISION = '6.3.39',
    ...
    jQuery(document).ready(function(){
        jQuery("#hosted-select").change(function(){
            var isDropIn = jQuery(this).val()==2;
            jQuery("#paypal-checkbox").parent().parent().toggle(isDropIn);
            jQuery("#paypal-credit").parent().parent().toggle(isDropIn);
            jQuery("#3ds_disabled").parent().parent().toggle(isDropIn);
        }).change();
    });
CUT1
        );
    ...
    function _doBill(Invoice $invoice, $doFirst, CcRecord $cc, Am_Paysystem_Result $result)
    {
        if ($doFirst) {
            ...
            // Recurring billing - use existing customer
    ...
    {
        if ($this->getConfig('hosted') != self::MODE_DROP_IN_UI) {
            $form->addStatic('_cc_info', ['class' => 'am-no-label'])
                ->setContent(<<<CONT
                    <div class="am-error">Not Supported. Please switch plugin to Drop-in Mode</div>
CONT
                );
            return;
...
CUT
        ...
        $vars = [];
        if ($id = $this->getConfig('merchant_account_id'))
        ...
        $form->addStatic('_cc_info', ['class' => 'am-no-label'])
            ->setContent(<<<CONT
                    <div id="dropin-container" class="paysystem-toggle paysystem-toggle-{$pluginId}"></div>
CONT
            );
        $amount = $this->invoice ? ($this->invoice->isFirstPayment() ? doubleval($this->invoice->first_total) : doubleval($this->invoice->second_total)) : '1.00';
        $paypal = $paypalCredit = $threeds = '';
        $form->addHidden($this->getJsTokenFieldName(), ['id' => $this->getJsTokenFieldName()]);
        if ($this->getConfig('paypal')) {
            $paypal = <<<CUT
    , paypal: {
        flow: 'vault'
    }
CUT;
        ...
        if ($this->getConfig('paypal_credit')) {
            $paypalCredit = <<<CUT
    , paypalCredit: {
        flow: 'vault'
    }
CUT;
        ...
        if (!$this->getConfig('3ds_disabled'))
            $threeds = <<<CUT
        threeDSecure: {
            amount : '{$amount}'
        },
CUT;
        $tokenFieldName = $this->getJsTokenFieldName();
        $form->addScript()->setScript(<<<CUTS
jQuery(document).ready(function(){
    var frm = jQuery("#cc");
    var submit = frm.find("input[type=submit]");
    frm.find('#{$tokenFieldName}').val("");
    ...
    braintree.dropin.create({
        ...
        jQuery('#dropin-container').closest('form').on("amFormSubmit", function(event){
            const frm = jQuery(event.target);
            const callback = event.callback;
            event.callback = null; // wait for completion
            if(jQuery("#dropin-container").closest('.am-row').is(':visible')){
            console.log(frm.find('#{$tokenFieldName}').val());
            
            if(frm.find('#{$tokenFieldName}').val()) callback();
            console.log(frm.find('#{$tokenFieldName}').val());
            instance.requestPaymentMethod(function (requestPaymentMethodErr, payload) {
                ...
                frm.find('#{$tokenFieldName}').val(payload.nonce);
...
CUTS
    ...
    function onInvoiceBeforePaymentSignup(Am_Event $event)
    {
        ...
        if ($this->isCCBrickSupported() && !empty($vars) && !empty($vars[$this->getJsTokenFieldName()])) {
            ...
            try {
                ...
                $cc->nonce = $vars[$this->getJsTokenFieldName()];
                $cc->user_id = $invoice->user_id;
                $result = new Am_Paysystem_Result();

                $this->_doBill($invoice, true, $cc, $result);
    ...
    {
        return $this->getConfig('hosted') == self::MODE_DROP_IN_UI;
    }
    ...
    function preDispatch()
    {
        ...
        if ($this->plugin->getConfig('hosted') == Am_Paysystem_Braintree::MODE_HOSTED_FORM) {
            $this->view->headScript()
                ->appendFile('https://js.braintreegateway.com/web/3.134.0/js/client.min.js')
                ->appendFile('https://js.braintreegateway.com/web/3.134.0/js/hosted-fields.min.js')
                ->appendFile('https://js.braintreegateway.com/web/3.134.0/js/paypal.min.js')
                ->appendFile('https://js.braintreegateway.com/web/3.134.0/js/data-collector.min.js');
  ...
  font-size: 14px;
    ...
    function updateAction()
    {
        ...
        if($this->getParam('credit_card__billing_address__street_address'))
            ...
            try {
                $result = \Braintree\Customer::update($bid, $vars);
            } catch (\Braintree\Exception $e) {
                $this->getDi()->logger->error("Braintree exception in updateAction", ["exception" => $e]);
                $this->view->error = ___("Payment gateway error occurred. Please try again.");
                $this->view->form = $this->form;
    ...
    function run(Am_Paysystem_Result $result)
    {
        ...
        try {
            ...
            if ($e instanceof \Braintree\Exception\Authorization) {
                $errorMessage = ___("Payment gateway authentication failed. Please contact site administrator.");
            } elseif ($e instanceof \Braintree\Exception\Authentication) {
                $errorMessage = ___("Payment gateway authentication failed. Please contact site administrator.");
            } elseif ($e instanceof \Braintree\Exception\NotFound) {
                $errorMessage = ___("Payment resource not found. Please try again.");
            } elseif ($e instanceof \Braintree\Exception\ServerError) {
                $errorMessage = ___("Payment gateway server error. Please try again later.");
            } elseif ($e instanceof \Braintree\Exception\Timeout) {
                $errorMessage = ___("Payment gateway request timeout. Please try again.");
            }
            $result->setFailed($errorMessage);
            return;
        } catch (Exception $e) {
            $log->add($e);
            $result->setFailed(___("Payment processing error occurred. Please try again."));
            return;
        }

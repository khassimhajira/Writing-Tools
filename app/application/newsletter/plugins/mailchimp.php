...
CUT
    ...
    function getApi(): Am_Mailchimp_Api
    {
    ...
    function onGridProductInitForm(Am_Event_Grid $event)
    {
        ...
        $_ = $this->getDi()->cacheFunction->call([$this, 'getListTagOptions'], [], ['id' => __CLASS__ . __FUNCTION__], 12 * 3600);

        $form->addMagicSelect('_mailchimp_segments', ['class' => 'am-combobox'])
            ->setLabel('Mailchimp List Segments
    ...
    function getTagOptions($list_id)
    {
        ...
        }, [$list_id], ['id' => __CLASS__ . __FUNCTION__ . md5($list_id)], 60);
    ...
    public function onUserAfterInsert(Am_Event_UserAfterInsert $e)
    {
        ...
        if (
            $this->getConfig('ecommerce')
            && $this->getDi()->request->getCookie('mc_cid')
            && $this->getDi()->request->getCookie('mc_eid')
        ) {
            $user = $e->getUser();
            $user->data()
    ...
    {
        return $user->data()->get($name) ?: $this->getDi()->request->getCookie($name);
    }
    ...
    public function onGetFlexibleActions(Am_Event $event)
    {
        try
        {
            $tags = $this->getDi()->cacheFunction->call([$this, 'getListTagOptions'], [], ['id' => __CLASS__ . __FUNCTION__], 60);

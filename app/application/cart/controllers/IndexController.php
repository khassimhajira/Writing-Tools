...
//        $this->addFromRequest();
    ...
    {
        return $this->_response->redirectLocation($this->getDi()->url(
            'login',
            ['saved_form' => 'cart','_amember_redirect_url' => base64_encode($this->view->overrideUrl())],
            false
        ));
    }
    ...
    public function ajaxRemoveOnlyAction()
    {
        ...
        try {
            ...
            foreach ($data as $item) {
                ...
                if ($i = $this->getCart()->getInvoice()->findItem(empty($item->type) ? 'product' : $item->type, $item_id))
                {

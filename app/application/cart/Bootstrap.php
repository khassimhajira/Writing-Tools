...
class Bootstrap_Cart extends Am_Module
{
    ...
    const EVENT_CART_GET_BILLING_PLANS = 'cartGetBillingPlans';
    ...
    public function getProductsQuery(?ProductCategory $category = null)
    {
        ...
        $hide_pids = array_diff(
            array_map(fn($p) => $p->pk(), $allCartProducts),
            array_map(fn($p) => $p->pk(), $filtered)
        );
...
CUT;
    ...
    function checkPath($path, $grid)
    {
        ...
        return !$this->getDi()->db->selectCell(
            "SELECT COUNT(*) FROM ?_product WHERE path=? {AND product_id<>?}",
            $path,
            $record->isLoaded() ? $record->pk() : DBSIMPLE_SKIP
        );
    }
    ...
    function onGridProductInitForm(Am_Event $e)
    {
        ...
        $fs->addUpload('img', null, ['prefix' => self::UPLOAD_PREFIX])
            ...
            ->setAllowedMimeTypes(['image/png', 'image/jpeg', 'image/gif', 'image/webp']);

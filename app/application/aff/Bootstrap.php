...
class Bootstrap_Aff extends Am_Module
{
    ...
    function createWidgetTopAffiliateConfigForm()
    {
        ...
        $form->addAdvCheckbox('all_tiers')
            ->setLabel(___("Use Commission for all tiers to rank affiliates\n" .
    ...
    function onUserMerge(Am_Event $event)
    {
        ...
        $this->getDi()->db->query('UPDATE ?_aff_click SET aff_id=? WHERE aff_id=?',  $target->pk(), $source->pk());
        $this->getDi()->db->query('UPDATE ?_aff_commission SET aff_id=? WHERE aff_id=?', $target->pk(), $source->pk());
        $this->getDi()->db->query('UPDATE ?_aff_lead SET aff_id=? WHERE aff_id=?', $target->pk(), $source->pk());
        $this->getDi()->db->query('UPDATE ?_aff_payout_detail SET aff_id=? WHERE aff_id=?', $target->pk(), $source->pk());
        $this->getDi()->db->query('UPDATE ?_user SET aff_id=? WHERE aff_id=?', $target->pk(), $source->pk());
    }
...
CUT
        ...
        if (!empty($user->aff_id)) {
            try {
                ...
                $affHtml = sprintf(
                    '<div><a class="link" href="%s">%s %s (%s)</a> <a href="javascript:;" title="%s" style="text-decoration:none; color:#ba2727" id="aff-unassign-affiliate">&#10005;</a>%s</div>',
                        Am_Html::escape($url->userUrl($user->aff_id)),
                        Am_Html::escape($aff->name_f),
                        Am_Html::escape($aff->name_l),
                        Am_Html::escape($aff->email),
                        ___('Unassign Affiliate'),
...
//        }
        ...
        if ($this->getConfig('mail_sale_admin')) {
            if ($et = Am_Mail_Template::load('aff.mail_sale_admin')) {
                $et->setPayment($commission->getPayment())
                    ...
                    ->setProduct($invoice_item->item_type == 'product' ? $this->getDi()->productTable->load($commission->product_id, false) : null)
 ...
 AND v.commission_id IS NOT NULL");
    ...
    public function onMailSimpleTemplateBeforeParse(Am_Event $event)
    {
        ...
        foreach (['user', 'affiliate'] as $prefix) {
            if (strpos($body . $subject, "%$prefix.aff_link%") !== false) {
    ...
    public function onInitBlocks(Am_Event $event)
    {
        ...
        $event->getBlocks()->add('aff/links/middle', new Am_Widget_AffCustomRedirect);
...
EOT;
    ...
    function getTrackingLink($aff, $banner_id = null, $escape = true, $params = [])
    {
        ...
        return $this->getDi()->surl($url, $params, $escape);
    }
    ...
    function getCustomTrackingLink($aff, $url, $escape = true)
    {
        if (is_object($aff))
            $aff = $aff->login;
        return $this->getDi()->surl('aff/go/' . urlencode($aff), ['cr' => base64_encode($url)], $escape);
    }
    ...
    function getBannerJs($aff, $banner_id)
    {
        if (is_object($aff))
            $aff = $aff->login;
        $url = $this->getDi()->surl('b/' . $this->getDi()->security->obfuscate($banner_id) . '/' . urlencode($aff));
    ...
    function logClick()
    {
        ...
        if ($this->aff) {
            ...
            $this->setCookie($this->aff, $this->banner ?: null, $aff_click_id);
        ...
        if ($link > 0 ) {
            ...
            if($this->aff) {
                if($custom_url = $this->getDi()->request->getParam('cr')) {
                    $cr = $this->getConfig('custom_redirect');
                    if (
                        ($cr == Bootstrap_Aff::AFF_CUSTOM_REDIRECT_ALLOW_SOME_DENY_OTHERS && $this->aff->aff_custom_redirect)

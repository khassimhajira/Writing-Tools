...
class AffCommissionRule extends Am_Record
{
    ...
    const COND_N_TIME_PAYMENT = 'n_time_payment';
    const COND_UPGRADE = 'upgrade';
    const COND_INVOICE_PAYMENT_NUM = 'invoice_payment_num';
    const COND_INVOICE_STARTED = 'invoice_started';
    const COND_USER_ACCESS_NUM = 'user_access_num';
...
CUT
                ...
                case self::COND_INVOICE_PAYMENT_NUM :
                    ...
                    break;
                case self::COND_USER_ACCESS_NUM :
                    if ($paymentNumber > 0) {
                        $num = intval($paymentNumber) - 1;
                        [$payment] = $this->getDi()->invoicePaymentTable->selectObjects(
                            "SELECT * FROM ?_invoice_payment WHERE invoice_id=? ORDER BY invoice_payment_id ASC LIMIT $num, 1",
                            $invoice->pk()
                        );
                        $access = $this->getDi()->accessTable->findFirstBy([
                            'invoice_item_id' => $item->pk(),
                            'invoice_payment_id' => $payment->pk(),
                        ]);
                    } else {
                        $access = $this->getDi()->accessTable->findFirstBy([
                            'invoice_item_id' => $item->pk(),
                    ...
                    /* Number of access records before just inserted access record */
                    $accessNumber = $this->getDi()->accessTable->countBy([
                        'user_id' => $invoice->getUser()->pk(),
                        'product_id' => $item->item_id,
                        ['access_id', '<', $access->pk()]
                    ]);
                    switch($vars['op']) {
                        case '=':
                            if ($accessNumber != $vars['val']) return false;
                            break;
                        case '>':
                            if ($accessNumber <= $vars['val']) return false;
                            break;
                    ...
                    break;
                case self::COND_INVOICE_STARTED :
                    $tm_started = $invoice->tm_started ?: sqlTime($paymentDate);
                    if (!empty($vars['from']) && $tm_started <= $vars['from']) return false;
                    if (!empty($vars['to']) && $tm_started >= $vars['to']) return false;
                    break;
                default:
                    return false;
            }
        ...
        foreach ($this->getConditions() as $conditionType => $vars)
        {
            switch ($conditionType)
            {
                ...
                case self::COND_FIRST_TIME_PAYMENT:
                    ...
                    break;
                case self::COND_N_TIME_PAYMENT:
                    $ret[] = ___('N-th Time Purchase %s %d', $vars['op'], $vars['val']);
                    break;
                case self::COND_AFF_FIRST_TIME_PAYMENT:
                ...
                case self::COND_INVOICE_PAYMENT_NUM:
                    ...
                    break;
                case self::COND_USER_ACCESS_NUM:
                    $ret[] = ___('User already had number of same product %s %d', $vars['op'], $vars['val']);
                    break;
                case self::COND_INVOICE_STARTED:
                    if (!empty($vars['from']) && !empty($vars['to'])) {
                        $ret[] = ___('Invoice Started between %s and %s', $vars['from'], $vars['to']);
                    } elseif (!empty($vars['from'])) {
                        $ret[] = ___('Invoice Started after %s', $vars['from']);
                    } elseif (!empty($vars['to'])) {
                        $ret[] = ___('Invoice Started before %s', $vars['to']);
                    }
                    break;
                default:
                    return false;

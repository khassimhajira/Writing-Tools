...
class Aff_AdminController extends Am_Mvc_Controller
{
    ...
    function subaffTabAction()
    {
        ...
        $grid->addField('comm', ___('Commission'), true, 'right')
            ...
            ->addDecorator(new Am_Grid_Field_Decorator_Link($this->getDi()->url('aff/admin/comm-tab/user_id/{user_id}', false), '_top'));
...
CUT;
    ...
    function payoutDetailAction()
    {
        ...
        $grid->addField('amount', ___('Commission'))->setRenderFunction([$this, 'renderCommAmount']);
    ...
    function commTabAction()
    {
        ...
        $grid->setEventId('gridAffCommissionTab');
        $grid->setPermissionId(Bootstrap_Aff::ADMIN_PERM_ID);
        $grid->actionsClear();
        ...
        $grid->addCallback(Am_Grid_Editable::CB_AFTER_INSERT, function (&$values, $record, $grid)
        {
            if (@$values['_add_tiers']) {
                ...
                foreach ($aff_tiers as $tier => $aff_tier) {
                    $rules = [];
                    $topay_this = $this->getDi()->affCommissionRuleTable->calculate(
                        $this->getDi()->invoiceRecord,
                        $this->getDi()->invoiceItemRecord,
                        $aff_tier,
                        1,
                        $tier,
                        $topay_this,

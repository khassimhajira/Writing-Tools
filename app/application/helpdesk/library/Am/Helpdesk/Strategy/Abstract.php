...
abstract class Am_Helpdesk_Strategy_Abstract
{
    ...
    protected function _addUploadFromClipboard($form)
    {
        $form->addScript()
            ->setScript(<<<'CUT'
document.getElementById('am-helpdesk-msg-area').addEventListener('paste', (e) => {
    const params = jQuery('#attachments-0').data('params');
    const prefix = document.getElementById('attachments-0').dataset.prefix;
    const secure = document.getElementById('attachments-0').dataset.secure;
    let item = Array.from(e.clipboardData.items).find(x => /^image\//.test(x.type))
    if (item) {
        let index = sessionStorage.getItem('am-helpdesk-upload') ?? 1
        let ext = 'png';
        ...
        formData.append('upload[]', item.getAsFile(), `image${index}.${ext}`)

        let url = amUrl(params.urlUpload, 1);

        fetch(url, {
            method: "POST",
            body: formData,
        }).then((r) => r.json()).then((d) => {
            jQuery('#attachments-0').amUpload('addFile', d);
        });
        
        sessionStorage.setItem('am-helpdesk-upload', ++index);
        e.preventDefault();
    }
})
CUT
    ...
    protected function getDi(): Am_Di
    {
        return $this->_di;
    }
    ...
    public static function create(Am_Di $di): Am_Helpdesk_Strategy_Abstract
    {
    ...
    public function createNewTicketForm(): Am_Form
    {
...
CUT
        ...
        $content = $form->addTextarea('content', ['class' => 'am-row-wide am-el-wide', 'rows' => 12, 'id' => 'am-helpdesk-msg-area'])
...
CUT
    ...
    public function getAdminGravatar($message)
    {
        ...
        return $admin
            ? sprintf('<img src="%s" width="40" height="40" />', '//www.gravatar.com/avatar/' . md5(strtolower(trim($admin->email))) . '?s=40&d=mm')
            : '';
    }
    ...
    public function getUserGravatar($message)
    {
        ...
        return $user
            ? sprintf('<img src="%s" width="40" height="40" />', '//www.gravatar.com/avatar/' . md5(strtolower(trim($user->email))) . '?s=40&d=mm')
            : '';
    }
    ...
    public function getAdminAvatar($message)
    {
        $admin = $this->getAdmin($message->admin_id);
        return $admin
            ? sprintf('<img src="%s" width="40" height="40" />', $this->getDi()->url('misc/avatar/' . $admin->avatar))
            : '';
    }
    ...
    public function getUserAvatar($message)
    {
        $user = $this->getUser($message->getTicket()->user_id);
        return $user
            ? sprintf('<img src="%s" width="40" height="40" />', $this->getDi()->url('misc/avatar/' . $user->avatar))
            : '';
    }
    ...
    public function getTicketNavigation(HelpdeskTicket $ticket): Am_Navigation_Container
    {
    ...
    protected function getAdmin($admin_id): ?Admin
    {
        return $this->getDi()->adminTable->load($admin_id, false);
    }

    protected function getUser($user_id): ?User
    {

...
class Am_Protect_Wordpress extends Am_Protect_Databased
{
    ...
    protected $_display_errors_backup = null;
    ...
    protected $guessFieldsPattern = ['ID', 'user_login', 'user_pass', 'user_nicename', 'display_name'];
...
CUT
        ...
        $form->addSelect($configPrefix ."version", null, ['options' => ['6.8.0' => '6.8.0 and newer', 4 => '4.0 and newer', 3 => 'older than 4.0']])
...
CUT
    ...
    {
        return version_compare($this->getConfig('version'), '6.8.0') >= 0 ? SavedPassTable::PASSWORD_WORDPRESS_BCRYPT : SavedPassTable::PASSWORD_PHPASS;
    }
    ...
    public function loginUser(Am_Record $record, $password)
    {
        ...
        if (
            ...
            && $this->getConfig('network_add_to_blogs')
            && ($groups = $this->calculateNetworkGroups($user))
    ...
    public function getLoggedInRecord()
    {
        if (!($user_id = $this->getWP()->wp_validate_auth_cookie()))
        {
            ...
            if (
                ...
                || !$user_id = $this->getWP()->wp_validate_auth_cookie($_COOKIE[$logged_in_cookie], 'logged_in')
            ) {
                return;
            }
        }
    ...
    function getDisplayName(User $user)
    {
        ...
        {
            case 'name_f_name_l':
                return $user->name_f . ' ' . $user->name_l;
            case 'username':
            default:
                return $user->login;
        }
    ...
    public function onGlobalIncludes(Am_Event_GlobalIncludes $e)
    {
        ...
        if (defined('DISABLE_WP_INCLUDE') && DISABLE_WP_INCLUDE && !$this->getConfig('wp_learndash') && !$this->getConfig('network')) return;
        if ($this->isConfigured() && ($this->getConfig('use_wordpress_theme') || $this->getConfig('network') || $this->getConfig('wp_learndash')))
        {
            ...
            $this->_display_errors_backup = ini_get('display_errors');
    ...
    public function onGlobalIncludesFinished()
    {
        ...
        if ($this->isConfigured() && ($this->getConfig('use_wordpress_theme') || $this->getConfig('network') || $this->getConfig('wp_learndash')))
        {
            ini_set('display_errors', $this->_display_errors_backup);
    ...
    function startLayout(Am_View $view, $title, $safe_jquery_load = false, $body_class = '')
    {
        ...
        add_filter("wp_title", [$this, "addTitle"], 99);
        add_filter("pre_get_document_title", [$this, "addTitle"], 99);
    ...
    function calculateGroups(?User $user = null, $addDefault = false)
    {
        ...
        if (
            $this->getConfig('network')
            && $this->getConfig('network_add_to_blogs')
            && $this->calculateNetworkGroups($user)
        ) {
    ...
    class Am_Protect_Wordpress_Table extends Am_Protect_Table
    {
        ...
        function getWpLearndashGroups(Am_Record $record)
        {
            ...
            foreach ($this->_db->select("SELECT * FROM ?_postmeta WHERE meta_key = '_sfwd-courses'") as $meta) {
                ...
                if (
                    !empty($data['sfwd-courses_course_access_list'])
                    && ($ids = explode(',', $data['sfwd-courses_course_access_list']))
                    && @in_array($record->ID, $ids)
                ) {

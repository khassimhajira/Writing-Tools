...
CUT;
    ...
    function renderParentInvoices(Am_View $view): void
    {
...
class Am_Grid_Action_OtoPreview extends Am_Grid_Action_Abstract
{
    ...
    function process(Am_Form $f)
    {
        ...
        Am_Mvc_Response::redirectLocation($this->grid->getDi()->surl(
            'admin-one-time-offer/preview',
            ['id' => $this->grid->getRecord()->pk(), 'user_id' => $user->pk()],
            false
        ));
    }

    protected function createForm(): Am_Form_Admin
    {
        $f = new Am_Form_Admin;
...
';
    ...
    public function previewAction()
    {
        ...
        if (!$user_id = $this->_request->getInt('user_id'))
    ...
    public function createGrid(): Am_Grid_Editable
    {
    ...
    function createForm(): Am_Form_Admin
    {
        ...
        $sel = $form->addMagicSelect('conditions[product]', ['class' => 'am-combobox'])
        ...
        foreach ($this->getDi()->billingPlanTable->getOptions() as $k => $v)
            $bp['bp-' . $k] = ___('Billing Plan') . ': ' . $v;
        foreach ($this->getDi()->otoTable->getOptions() as $k => $v)
            $oto['oto-' . $k] = ___('OTO') . ': ' . $v;

        $options =
            ($cats ? [___('Categories') => $cats] : [])
            + ($pr ? [___('Products') => $pr] : [])
            + ($bp ? [___('Billing Plans') => $bp] : [])
...
class OtoTable extends Am_Table
{
    ...
    function findUpsell(array $products, Invoice $invoice): ?Oto
    {
        $pids = $bpids = [];
        ...
        foreach ($products as $p) {
            if ($p instanceof Product) {
                $pids[] = $p->product_id;
                $bpids[] = $p->getBillingPlan()->pk();
            }
        }

        foreach ($this->findByIsDisabled(0) as $oto) {
            /* @var $oto Oto */
            if (
                $oto->matchProducts($pids)
                || $oto->matchBillingPlans($bpids)
            ) {
                if (
                    $oto->skip_if_exists
    ...
    function findDownsell(int $oto_id, Invoice $invoice): ?Oto
    {
        foreach ($this->findByIsDisabled(0) as $oto) {
            /* @var $oto Oto */
    ...
    function getOptions(): array
    {
        return array_map(
            ['Am_Html', 'escape'],
            $this->_db->selectCol("SELECT oto_id as ARRAY_KEY, comment FROM ?_oto ORDER BY comment")
        );
    }
}
...
class Oto extends Am_Record
{
    ...
    function matchProducts(array $product_ids): bool
    {
    ...
    function matchBillingPlans(array $billing_plan_ids): bool
    {
        $cond = $this->getConditions();
        foreach ($cond['product'] as $s) {
            if (preg_match('/bp-(\d+)/', $s, $regs))
                if (in_array($regs[1], $billing_plan_ids))
                    return true;
        }
        return false;
    }

    function matchOto($oto_id): bool
    {
        $cond = $this->getConditions();
        foreach ($cond['product'] as $s) {
    ...
    {
        return $this->coupon_id ? $this->getDi()->couponTable->load($this->coupon_id) : null;
    }

...
class Am_Paysystem_Clickbank extends Am_Paysystem_Abstract
{
    ...
    {
        $e->getBlocks()->add('thanks/success', new Am_Block_Base('ClickBank Statement', 'clickbank-statement', $this, [$this, 'renederStatement']));
    }
...
CUT;
    ...
    public function _process($invoice, $request, $result)
    {
        ...
        foreach($invoice->getItems() as $item)
        {
            $items[] = $item->getBillingPlanData('clickbank_product_id') . '.' . $item->qty;
        }
        $a = new Am_Paysystem_Action_Redirect(sprintf($this->cartUrl, $this->getConfig('account')));
        $a->cbitems = implode('_', $items);

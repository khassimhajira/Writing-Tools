...
class Am_Security
{
     ...
     * @param int $len
     * @param string|array $acceptedChars ex. "abcdef1234" or ["abcdef", "12345"]
     *      array is used to guarantee presence of char from each set if length allow it
     * @return string
     */
    function randomString($len, $acceptedChars = null)
    {
        ...
        $security_code = "";
        for ($i = 0; $i < $len; $i++) {
            $chars = is_string($acceptedChars) ? $acceptedChars : $this->getAcceptedChars($acceptedChars, $used);
            $max = strlen($chars) - 1;
            $security_code .= $chars[random_int(0, $max)];
        }

        return $security_code;
    }

    protected function getAcceptedChars(array $acceptedChars, ?array &$alreadyUsed)
    {
        $alreadyUsed ??= [];

        if (count($acceptedChars) == count($alreadyUsed)) {
            $alreadyUsed = [];
        }

        $_ = array_values(array_filter($acceptedChars, fn($_) => !in_array($_, $alreadyUsed), ARRAY_FILTER_USE_KEY));

        $index = random_int(0, count($_) - 1);
        $alreadyUsed[] = array_search($_[$index], $acceptedChars);

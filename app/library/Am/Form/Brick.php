...
abstract class Am_Form_Brick
{
    ...
    {
        $this->customLabels = array_map(function($_){return preg_replace("/\r?\n/", "\r\n", $_);}, $labels);
    }
...
class Am_Form_Brick_JavaScript extends Am_Form_Brick
{
    ...
    public function insertBrick(HTML_QuickForm2_Container $form)
    {
        switch($this->getConfig('position', 'body')) {
            case 'head':
                Am_Di::getInstance()->view->headScript()->appendScript($this->getConfig('code'));
                break;
            case 'inline':
            default:
...
CUT
    ...
    public function insertBrick(HTML_QuickForm2_Container $form)
    {
        ...
        $pass = $form->addPassword('pass', ['autocomplete' => 'new-password', 'spellcheck' => 'false', 'maxlength' => Am_Di::getInstance()->config->get('pass_max_length', 64), 'class' => 'am-pass-indicator'])
        ...
        if (!$this->getConfig('do_not_confirm')) {
            $pass0 = $form->addPassword('_pass', ['autocomplete' => 'new-password', 'spellcheck' => 'false', 'maxlength' => Am_Di::getInstance()->config->get('pass_max_length', 64)])
...
CUT
        ...
        if (!$this->getConfig('do_not_ask_current_pass')) {
            $oldPass = $form->addPassword('_oldpass', ['autocomplete' => 'current-password', 'spellcheck' => 'false', 'class'=>'am-change-pass'])
                ->setLabel($this->___("Your Current Password\nif you are changing password, please\n enter your current password for validation"));
            $oldPass->addRule('callback2', 'wrong', [$this, 'validateOldPass']);
        }
        $pass = $form->addPassword('pass', ['autocomplete' => 'new-password', 'spellcheck' => 'false', 'maxlength' => Am_Di::getInstance()->config->get('pass_max_length', 64), 'class'=>'am-change-pass'])
                ->setLabel($this->___("New Password\nyou can choose new password here or keep it unchanged\nmust be %d or more characters", $len));
        $pass->addRule('callback2', 'wrong', [$this, 'validateNewPass']);
        $pass->addRule('length', sprintf($this->___('Password must contain at least %d letters or digits'), $len),
            [$len, Am_Di::getInstance()->config->get('pass_max_length', 64)]);
        ...
        if (!$this->getConfig('do_not_confirm')) {
            $pass0 = $form->addPassword('_pass', ['autocomplete' => 'new-password', 'spellcheck' => 'false', 'class'=>'am-change-pass'])
    ...
    public function validateNewPass($vars, HTML_QuickForm2_Element_InputPassword $el)
    {
        $vars = $el->getContainer()->getValue();
        if ($vars['pass'] != '') {
            if (Am_Di::getInstance()->user->checkPassword($vars['pass'])) {
                return $this->___('Your new password must be different from your current password');
            }
        }
    }
...
class Am_Form_Brick_Address extends Am_Form_Brick
{
    ...
    {
        $user = Am_Di::getInstance()->auth->getUser();
        $country_disabled = $user && $user->country && $this->getConfig('country_disabled');
        ...
        if (!$ro && $this->getConfig('country_default')) {
            ...
            if ($this->getConfig('state_default')) {
                $_['state'] = $this->getConfig('state_default');
            }

            $f->addDataSource(new HTML_QuickForm2_DataSource_Array($_));
        }
    }
    ...
    public function initConfigForm(Am_Form $form)
    {
        ...
        $form->addSelect('state_default')->setLabel('Default State')->loadOptions(Am_Di::getInstance()->stateTable->getOptions($this->getConfig('country_default')));
        $form->addAdvCheckbox('hide_fieldset')->setLabel(___('Hide Brick Title'));
        $form->addAdvCheckbox("disabled")->setLabel(___('Read-only'));

        $form->addAdvCheckbox('country_disabled')->setLabel(___("Disallow Country Change\nin event of user already fill in this field then he will not be able to alter it"));

        $form->addScript()
            ->setScript(<<<CUT
jQuery(function(){
    jQuery('#brick-config-address [name=country_default]').change(function(){
        jQuery('#brick-config-address [name=state_default]').closest('.am-row').toggle(this.value != '');
        const val = jQuery('#brick-config-address [name=state_default]').val();
        const state = jQuery('#brick-config-address [name=state_default]').get(0);
        jQuery('#brick-config-address [name=state_default]').empty();
        if (this.value) {
            jQuery.get(amUrl('/') + 'ajax?do=get_states&country=' + this.value, function(states) {
...
class Am_Form_Brick_Product extends Am_Form_Brick
{
    ...
    public function insertProductOptions(HTML_QuickForm2_Container $form, $pid, array $productOptions, BillingPlan $plan)
    {
        foreach ($productOptions as $option)
        {
            ...
            switch ($option->type)
            {
                ...
                case 'select':
                    ...
                    $el->loadOptions(['' => ''] + array_map('strip_tags', $option->getSelectOptionsWithPrice($plan, false)));
                    if ($isEmpty)
                        $el->setValue($option->getDefault());
                    break;
                case 'multi_select':
...
class Am_Form_Brick_Paysystem extends Am_Form_Brick
{
    ...
    public function insertBrick(HTML_QuickForm2_Container $form)
    {
        ...
        if (count($paysystems) != 1) {
            ...
            foreach ($psOptions as $k => $v) {
                $attrs = [
                    'data-recurring' => json_encode(defined('AM_ALL_PAYSYS_RECURRING') || (bool)$psIndex[$k]->isRecurring())
                ];
    ...
    public function initConfigForm(Am_Form $form)
    {
        ...
        $form->addSortableMagicSelect('paysystems')
...
class Am_Form_Brick_Coupon extends Am_Form_Brick
{
    ...
    {
        $_ = parent::getRecord();
        $_['config']['coupon_default_begin'] = $_['config']['coupon_default_begin'] ? Am_Form_Element_Date::convertReadableToSQL($_['config']['coupon_default_begin']) : '';
        $_['config']['coupon_default_expire'] = $_['config']['coupon_default_expire'] ? Am_Form_Element_Date::convertReadableToSQL($_['config']['coupon_default_expire']) : '';
        return $_;
    }
    ...
    public function initConfigForm(Am_Form $form)
    {
        ...
        $dateGroup = $form->addGroup()
            ->setLabel(___("Default Coupon Dates\npopulate coupon field with default value within this date range"));
        $dateGroup->setSeparator(' ');
        $dateGroup->addDate('coupon_default_begin', ['placeholder' => ___('Begin Date')]);
        $dateGroup->addHtml()
            ->setHtml('&mdash;');
        $dateGroup->addDate('coupon_default_expire', ['placeholder' => ___('End Date')]);
...
jQuery(function(){
    ...
    jQuery('[name=coupon_default]').change(function(){
        jQuery('[name=coupon_default_begin]').closest('.am-row').toggle(this.value != '');
    }).change()
});
CUT
            );
...
CUT
        ...
        $coupon->addRule('callback2', '--error--', [$this, 'validateCoupon'])
            ->addRule('remote', '--error--', ['url' => $di->url('ajax', ['do' => 'check_coupon'], false)]);

        if (
            ($code = $this->getConfig('coupon_default'))
            && (empty($this->getConfig('coupon_default_begin')) || $this->getConfig('coupon_default_begin') <= sqlDate('now'))
            && (empty($this->getConfig('coupon_default_expire')) || $this->getConfig('coupon_default_expire') >= sqlDate('now'))
    ...
    function validateCoupon($code)
    {
        if ($code == "")
            return null;
        $coupon = Am_Di::getInstance()->couponTable->findFirstByCode($code);
        ...
        foreach ($agreements as $agreement)
        {
            ...
            if (!empty($agreement->url)) {
                ...
                $attrs = Am_Di::getInstance()->view->attrs([
                    'href' => "javascript:",
                    'class' => 'local-link',
                    'onclick' => 'jQuery("#' . $agreement->type . '").amPopup({width:600, title:' . $header . '});'
                ]);
            }
...
class Am_Form_Brick_VatId extends Am_Form_Brick
{
    ...
    public function validate($id)
    {
        ...
        $cacheKey = 'vc_' . preg_replace('/[^A-Z0-9a-z_]/', '_', $me) . '_' . preg_replace('/[^A-Z0-9a-z_]/', '_', $id);
        ...
        if ($country == 'GB') { //BREXIT
            //https://developer.service.hmrc.gov.uk/api-documentation/docs/api/service/vat-registered-companies-api/1.0/oas/page#tag/organisations
            $request = new Am_HttpRequest('https://api.service.hmrc.gov.uk/organisations/vat/check-vat-number/lookup/' . $number);
            $resp = $request->send();
            ...
            $request = new Am_HttpRequest('http://ec.europa.eu/taxation_customs/vies/services/checkVatService',
...
CUT
            ...
            if ($xml === false || count($xml->xpath("//*[local-name()='checkVatResponse']")) == 0) {
                return $this->___("Cannot validate VAT Id, please try again");
            }

...
CUT
        ...
        $fs = $this->addAdvFieldset('##04')
            ->setLabel(___('Coupon Code Configuration'));

        $fs->addAdvCheckbox('coupon_no_special_chars')
            ->setLabel(___('Do not allow to use special chars in coupon code
            Only alphanumeric characters (a-z, A-Z, 0-9) are allowed
        '));
        $this->setDefault('coupon_min_length', 2);
        $this->setDefault('coupon_max_length', 16);
        $passLen = $fs->addGroup(null, null)->setLabel(___('Coupon Length'));
        $passLen->addInteger('coupon_min_length', ['size'=>3])->setLabel('min');
        $passLen->addStatic('')->setContent(' &mdash; ');
        $passLen->addInteger('coupon_max_length', ['size'=>3])->setLabel('max');
...
class Am_Form_Setup_Email extends Am_Form_Setup
{
    ...
    function initElements()
    {
        ...
        $emailing_methods = [
            ...
            'mailgun' => ___('MailGun'),
            'gmailoauth' => ___('Gmail OAUTH'),
            'disabled' => ___('Disabled'),
        ];
        $event = new Am_Event(Am_Event::GET_EMAILING_METHODS);
        $this->getDi()->hook->call($event);
        foreach($event->getReturn() as $m){
            $emailing_methods += $m;
        }
        ...
        $fs->addSelect('ses_region', '', [
            'options' => [
                ...
                Am_Mail_Transport_Ses::REGION_US_WEST_1 => 'US West (N. California)',
                Am_Mail_Transport_Ses::REGION_US_WEST_2 => 'US West (Oregon)',
                Am_Mail_Transport_Ses::REGION_AP_SOUTH_1 => 'Asia Pacific (Mumbai)',
                Am_Mail_Transport_Ses::REGION_AP_SOUTHEAST_2 => 'Asia Pacific (Sydney)',
                Am_Mail_Transport_Ses::REGION_AP_NORTHEAST_1 => 'Asia Pacific (Tokyo)',
        ...
        $fs->addText('ses_configuration_set', ['class' => 'am-el-wide'])
            ->setLabel(___("Amazon SES Configuration Set\nkeep empty if not used"));
        ...
        $fs->addText('gmailoauth_client_id', ['class' => 'am-el-wide'])
            ->setLabel(___('Gmail OAUTH Client ID'))
            ->addRule('required', ___('This field is required'), null, HTML_QuickForm2_Rule::CLIENT);

        $fs->addPassword('gmailoauth_client_secret', ['class' => 'am-el-wide'])
            ->setLabel(___('Gmail OAUTH Client Secret'))
            ->addRule('required', ___('This field is required'), null, HTML_QuickForm2_Rule::CLIENT);

        $fs->addPassword('gmailoauth_refresh_token', ['class' => 'am-el-wide'])
            ->setLabel(___('Gmail OAUTH Refresh Token'))
            ->addRule('required', ___('This field is required'), null, HTML_QuickForm2_Rule::CLIENT);

        $this->getDi()->hook->call(Am_Event::EMAILING_METHODS_INIT_FORM, [
            'fieldset' => $fs
        ]);
...
CUT
        ...
        $emailing_methods_js = '';
        foreach($emailing_methods as $id => $_)
        {
            if(in_array($id, ['mail', 'disabled'])) continue;
            $emailing_methods_js .= <<<CUT
            jQuery(".am-row[id*='{$id}_']").toggle(jQuery(this).val() == '{$id}');
CUT
;
        }
...
CUT
        ...
        $fs->addMagicSelect('allow_restore_pids', ['class' => 'am-combobox-fixed'])
            ->setLabel(___("Allow resume cancelled recurring subscription for Products\nkeep empty to allow for any"))
            ->loadOptions($this->getDi()->productTable->getProductOptions());

        $fs->addScript()->setScript(<<<CUT
jQuery(function(){
    jQuery('[name=allow_restore]').change(function(){
        jQuery('[data-name^=allow_restore_pids]').closest('.am-row').toggle(this.checked);
    }).change();
});
CUT
        );
        ...
        $fs->addMagicSelect('allow_cancel_pids', ['class' => 'am-combobox-fixed'])
            ->setLabel(___("Allow cancel recurring subscription for Products\nkeep empty to allow for any"))
            ->loadOptions($this->getDi()->productTable->getProductOptions());

        $fs->addScript()->setScript(<<<CUT
jQuery(function(){
    jQuery('[name=allow_cancel]').change(function(){
        jQuery('[data-name^=allow_cancel_pids]').closest('.am-row').toggle(this.checked);
    }).change();
});
CUT
        );
...
CUT
        ...
        $gr->addStatic()->setContent(' ' . ___('times then he will be forced to wait') . ' ');
        $this->setDefault('bruteforce_delay', '120');
        $gr->addInteger('bruteforce_delay', ['size' => 5]);
        $gr->addStatic()->setContent(' ' . ___('seconds until next try'));
        $gr->addStatic()->setContent('</div>');
...
class Am_Form_Setup_PersonalData extends Am_Form_Setup
{
    ...
    function initElements()
    {
        ...
        $fs->addMagicSelect('personal-data-download-fields', ['class' => 'am-combobox', 'id'=>'download-fields'])
            ->setLabel(___("These fields will be included in XML document\n"

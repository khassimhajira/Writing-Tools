...
class Am_Grid_DataSource_CustomField extends Am_Grid_DataSource_Array
{
    ...
    public function deleteRecord($id, $record)
    {
        ...
        if (in_array($record->type, ['upload', 'multi_upload'])) {
            if ($record->sql) {
    ...
    protected function getRecordForStore($values)
    {
        ...
        $options = [];
        foreach ($values['values']['options'] as $k => $v) {
            $options[trim($k)] = $v;
        }
        ...
        $recordForStore['additional_fields'] = [
    ...
    {
        $this->di->db->query(<<<CUT
INSERT INTO ?_data (`table`, `key`, `id`, `type`, `value`, `blob`)

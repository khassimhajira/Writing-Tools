...
class Am_Grid_ReadOnly
{
    ...
    const CB_RENDER_ROW = 'onRenderRow';
     ...
     */
    function addField($field, $title = null, $sortable = null, $align = null, $renderFunc = null, $width = null)
    {
    ...
    function addFieldBefore($ref, $field, $title = null, $sortable = null, $align = null, $renderFunc = null, $width = null)
    {
    ...
    function addFieldAfter($ref, $field, $title = null, $sortable = null, $align = null, $renderFunc = null, $width = null)
    {
        if (func_num_args()>2 || !$field instanceof Am_Grid_Field) {
            //first element is reference
            $field = $this->_createField(array_slice(func_get_args(), 1));
        ...
        foreach ($this->fields as $k => $f) {
            if ($f->getFieldName() == $ref_name) {
                array_splice($this->fields, $k+1, 0, [$field]);
                $field->init($this);
                return $field;
            }
        }
    ...
    function prependField($field, $title = null, $sortable = null, $align = null, $renderFunc = null, $width = null)
    {
        if (func_num_args()>1 || !$field instanceof Am_Grid_Field) {
            $field = $this->_createField(func_get_args());
        }
    ...
    function renderTitle($noTags = false)
    {
        ...
        $msgs = [];
        $msgs[] = $this->isAsyncTotals()
            ? $this->renderTotalPlaceholder()
    ...
    {
        return [
            'totalRecords' => $this->isAsyncTotals() ? null : $this->getFoundRows(),
    ...
    function renderTable()
    {
        ...
        $attr = $this->isAsyncTotals() ? [
            'data-storage' => 'info',
            'data-url' => $this->makeUrl([self::ACTION_KEY => 'reload']),
            'data-key' => 'totalRecords',
        ] : [];

        $out .= '<div class="am-grid-container">'.PHP_EOL;
        $out .= sprintf(
            '<table class="am-grid%s" data-info="%s" %s>'.PHP_EOL,
            ($this->isAsyncTotals() ? ' need-reload' : ''),
            $this->escape(json_encode($this->getHiddenVars())),
            Am_Html::attrs($attr)
        );
        $out .= "\t<thead><tr>\n";
        ...
        foreach ($records as $record) {
            $row = $this->renderRow($record);
            $args = [& $row, $record, $records, $this];
            $this->runCallback(self::CB_RENDER_ROW, $args);
            $out .= $row;
        }
        $out .= "</tbody></table></div>\n\n";
        // run callback
    ...
    function renderRow($record)
    {
        ...
        foreach ($attribs as $k => $v)
            $astring .= ' ' . htmlentities($k, 0, 'UTF-8') . '="' . htmlentities($v, ENT_QUOTES, 'UTF-8').'"';
    ...
    protected function renderTotalPlaceholder()
    {
        return sprintf(
            "<span class='need-reload' data-url='%s' data-key='total'>%s</span>",
            $this->makeUrl([self::ACTION_KEY => 'reload']),
            ''
        );
    }

    protected function renderPaginatorPlaceholder()
    {
        return sprintf("<span class='need-reload' data-url='%s' data-key='paginator'>%s</span>",
            $this->makeUrl([self::ACTION_KEY => 'reload']),
            ''
        );
    }
    ...
    function renderContent()
    {
        ...
        $out =
            $this->renderFilter() .
            $table .
            ($this->isAsyncTotals() ? $this->renderPaginatorPlaceholder() : $this->renderPaginator($this->canCacheTotals()));

        // run callback
        $args = [& $out, $this];
        $this->runCallback(self::CB_RENDER_CONTENT, $args);
        // done
        return $this->renderTitle() . $out;
    }
    ...
    {
        $desc = null;
        @[$sort, $desc] = explode(' ', $this->request->getParam('sort'), 2);
    ...
    static function renderTd($content, $doEscape = true, $attr = [])
    {
        return sprintf('<td%s>%s</td>',
            $attr ? ' ' . Am_Html::attrs($attr) : '',
    ...
    {
        echo $this->isAjax() ? $this->renderContent() : $this->render();
    }
    ...
    function reloadAction()
    {
        ...
        $this->getDi()->response->ajaxResponse([
            'totalRecords' => $this->getFoundRows(false),
    ...
    function getView()
    {
        return $this->view ?: $this->getDi()->view;
    }
    ...
    {
        return $this->permissionId ?: 'grid' . $this->id;
    }
    ...
    {
        return AM_HUGE_DB && !AM_HUGE_DB_NO_CACHE && method_exists($this->getDataSource(), 'getCacheId');
    }

    function isAsyncTotals()
    {
        return AM_HUGE_DB;
    }
    ...
    function getFoundRows($fromCache = true)
    {
        if (
            $this->canCacheTotals()
            && $fromCache
            && ($_ = $this->getDi()->cache->load($this->getDataSource()->getCacheId('long-time')))
        ) {
            return $_;
        }

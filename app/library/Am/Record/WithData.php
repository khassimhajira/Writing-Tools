...
abstract class Am_CustomField
{
    ...
    static function create($name, $title, $type = 'text', $description = null, $validateFunc = null, $moreParams = [])
    {
        ...
        return new $className($name, $title, $description, $validateFunc, $moreParams);
    }
    ...
    function valueFromTable($val)
    {
        if (!$this->isArray)
            return $val;
        return is_array($val) ? $val : (array) unserialize($val ?: 'a:0:{}');
    }
...
class Am_CustomFieldUpload extends Am_CustomField
{
    ...
    public function addToQF2(HTML_QuickForm2_Container $container, $attr = [], $data = [], $runAt = HTML_QuickForm2_Rule::CLIENT_SERVER)
    {
        ...
        $el = parent::addToQF2($container, $attr, array_merge($data, [
            ...
            'urlGet' => defined('AM_ADMIN') && AM_ADMIN ? 'admin-upload/get' : 'upload/get',
        ]), $runAt);
...
class Am_DataFieldStorage
{
        ...
        foreach ($this->data as $k => $arr) {
            switch ($arr['type']) {
                case self::TYPE_SCALAR:
                    $this->data[$k] = $arr['value'];
                    break;
                case self::TYPE_SERIALIZED:
                    $this->data[$k] = unserialize($arr['value']);
                    break;
                case self::TYPE_BLOB:
                    $this->data[$k] = self::BLOB_VALUE;
                    break;
                default:
    ...
    function update()
    {
        ...
        if ($fields)
            $this->record->getAdapter()->query(
                "DELETE FROM ?_data WHERE `table`=? AND `id`=? AND `key` IN (?a)",
                $this->record->getTable()->getName(true),
                $this->record->pk(),
                $fields
            );
    ...
    {
        $this->record->getAdapter()->query(
            "DELETE FROM ?_data WHERE `table`=? AND `id`=?",
            $this->record->getTable()->getName(true),
            $this->record->pk()
        );
        $this->data = [];
...
class Am_Table_WithData extends Am_Table
{
    ...
    {
        return $this->_customFieldsConfigKey ?: $this->getName(true) . '_fields';
    }
    ...
    function findByData($key, $value, $limit = 100)
    {
        return $this->selectObjects(<<<CUT
                SELECT t.* FROM {$this->_table} t
                    ...
                    WHERE
                        {IF(d.type = 0, d.`value`, d.`blob`) IN (?a)}
                        {IF(d.type = 0, d.`value`, d.`blob`) = ?}
                    {LIMIT ?d}
CUT
                ,
                $this->getName(true),
    ...
    /**
     * Find first record by related ?_data record
     * @param string $key
     * @param string $value

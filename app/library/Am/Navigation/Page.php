...
abstract class Am_Navigation_Page extends Am_Navigation_Container
{
    /**
     * Page label
     *
     ...
     *
     * @var string|Am_Acl_Resource_Interface|null
     */
    protected $_resource;
     ...
     * Parent container
     *
     * @var Am_Navigation_Container|null
     */
    protected $_parent;
    ...
    /**
     * Factory for Am_Navigation_Page classes
     *
     ...
     * to Am_Navigation_Page_Uri or Am_Navigation_Page_Mvc. Any other value
     * for 'type' will be considered the full name of the class to construct.
     * A valid custom page class must extend Am_Navigation_Page.
     *
     * If 'type' is not given, the type of page to construct will be determined
     * by the following rules:
     * - If $options contains either of the keys 'action', 'controller',
     *   'module', or 'route', a Am_Navigation_Page_Mvc page will be created.
     * - If $options contains the key 'uri', a Am_Navigation_Page_Uri page
     *   will be created.
     *
     * @param  array|Am_Config $options  options used for creating page
     * @return Am_Navigation_Page        a page instance
     * @throws Am_Navigation_Exception   if $options is not array/Am_Config
     * @throws Am_Exception              if 'type' is specified and
     *                                     Am_Loader is unable to load the
     *                                     class
     * @throws Am_Navigation_Exception   if something goes wrong during
     *                                     instantiation of the page
     * @throws Am_Navigation_Exception   if 'type' is given, and the specified
     *                                     type does not extend this class
     * @throws Am_Navigation_Exception   if unable to determine which class
     *                                     to instantiate
     */
    public static function factory($options)
    {
        if ($options instanceof Am_Config) {
        ...
        if (!is_array($options)) {
            ...
            throw new Am_Navigation_Exception(
                'Invalid argument: $options must be an array or Am_Config');
        }
        ...
        if(isset($type)) {
            if (is_string($type) && !empty($type)) {
                ...
                if (!class_exists($type)) {
                    ...
                    @Am_Loader::loadClass($type);
                }

                $page = new $type($options);
                if (!$page instanceof Am_Navigation_Page) {
                    //--//require_once 'Zend/Navigation/Exception.php';
                    throw new Am_Navigation_Exception(sprintf(
                            'Invalid argument: Detected type "%s", which ' .
                            'is not an instance of Am_Navigation_Page',
                            $type));
                }
                return $page;
            }
        ...
        if ($hasMvc) {
            ...
            return new Am_Navigation_Page_Mvc($options);
        } elseif ($hasUri) {
            //--//require_once 'Zend/Navigation/Page/Uri.php';
            return new Am_Navigation_Page_Uri($options);
        } else {
            //--//require_once 'Zend/Navigation/Exception.php';
            ...
            throw new Am_Navigation_Exception($message);
    }
    }
     ...
     * Page constructor
     *
     * @param  array|Am_Config $options   [optional] page options. Default is
     *                                      null, which should set defaults.
     * @throws Am_Navigation_Exception    if invalid options are given
     */
    public function __construct($options = null)
    {
        if (is_array($options)) {
            $this->setOptions($options);
        } elseif ($options instanceof Am_Config) {
    ...
    /**
     * Sets page properties using a Am_Config object
     *
     * @param  Am_Config $config        config object to get properties from
     * @return Am_Navigation_Page       fluent interface, returns self
     * @throws Am_Navigation_Exception  if invalid options are given
     */
    public function setConfig(Am_Config $config)
    {
     ...
     * @return Am_Navigation_Page       fluent interface, returns self
     * @throws Am_Navigation_Exception  if empty/no string is given
     */
     ...
     *                                    which sets no id.
     * @return Am_Navigation_Page       fluent interface, returns self
     * @throws Am_Navigation_Exception  if not given string or null
     */
     ...
     * @return Am_Navigation_Page       fluent interface, returns self
     * @throws Am_Navigation_Exception  if target is not string or null
     */
     ...
     * @return Am_Navigation_Page       fluent interface, returns self
     * @throws Am_Navigation_Exception  if access key is not string or null or
     ...
     *
     * @param  array|Am_Config $relations  [optional] an associative array of
     *                                       forward links to other pages
     * @return Am_Navigation_Page          fluent interface, returns self
     */
    public function setRel($relations = null)
    {
        ...
        if (null !== $relations) {
            if ($relations instanceof Am_Config) {
            ...
            if (!is_array($relations)) {
                ...
                throw new Am_Navigation_Exception(
                        ...
                        'array or an instance of Am_Config');
            }
     ...
     * @return Am_Navigation_Page         fluent interface, returns self
     * @throws Am_Navigation_Exception    if name is not string or value is
     ...
     * @throws Am_Navigation_Exception    if name is not string
     */
     ...
     *                                    specific order.
     * @return Am_Navigation_Page       fluent interface, returns self
     * @throws Am_Navigation_Exception  if order is not integer or null
     */
     ...
     *
     * @param  string|Am_Acl_Resource_Interface $resource  [optional] resource
     *                                                       to associate with
     *                                                       page. Default is
     *                                                       null, which sets no
     *                                                       resource.
     * @throws Am_Navigation_Exception                     if $resource if
     *                                                       invalid
     * @return Am_Navigation_Page                          fluent interface,
     *                                                       returns self
     */
    public function setResource($resource = null)
    {
        if (null === $resource || is_string($resource) ||
            $resource instanceof Am_Acl_Resource_Interface) {
            $this->_resource = $resource;
        } else {
            //--//require_once 'Zend/Navigation/Exception.php';
            throw new Am_Navigation_Exception(
                    'Invalid argument: $resource must be null, a string, ' .
                    ' or an instance of Am_Acl_Resource_Interface');
        }
     ...
     *
     * @return string|Am_Acl_Resource_Interface|null  ACL resource or null
     */
    ...
    public function isVisible($recursive = false)
    {
        if ($recursive && isset($this->_parent) &&
            $this->_parent instanceof Am_Navigation_Page) {
     ...
     *
     * @param  Am_Navigation_Container $parent  [optional] new parent to set.
     *                                            Default is null which will set
     *                                            no parent.
     * @return Am_Navigation_Page               fluent interface, returns self
     */
    public function setParent(?Am_Navigation_Container $parent = null)
    {
        if ($parent === $this) {
            //--//require_once 'Zend/Navigation/Exception.php';
            throw new Am_Navigation_Exception(
     ...
     *
     * @return Am_Navigation_Container|null  parent container or null
     */
     ...
     * @return Am_Navigation_Page       fluent interface, returns self
     * @throws Am_Navigation_Exception  if property name is invalid
     */
     ...
     * @param  string $name               property name
     * @return void
     * @throws Am_Navigation_Exception  if the property is native
     */

...
class Am_Blocks
{
     ...
     * @return \Am_Blocks
     */
    function add($target, $block = null, $order = self::MIDDLE): Am_Blocks
    {
        // for compatibility
    ...
    function remove(string $id): Am_Blocks
    {
    ...
    function replace(string $id, Am_Block_Base $block): Am_Blocks
    {
        foreach ($this->blocks as $k => $target)
            foreach ($target as $kk => $b)
                if ($b['block']->getId() == $id)
                    $this->blocks[$k][$kk]['block'] = $block;
        return $this;
    }

    function setOrder(string $id, int $order): Am_Blocks
    {
        foreach ($this->blocks as $k => & $target)
            foreach ($target as $kk => & $block)
                if ($block['block']->getId() == $id)
     ...
     */
    function getBlock(string $id): ?Am_Block_Base
    {
        foreach ($this->blocks as $k => $target)
            foreach ($target as $kk => $block)
                if ($block['block']->getId() == $id)
...
class Am_Laz
{
    ...
    /**
     * This function intended to avoid immediate object initialization in Di
    ...
    {
        return sprintf('di->%s->%s', $this->diObj, $this->method);
    }
    ...
    function read()
    {
        ...
        try {
            ...
            if (function_exists('am_set_config')) {
                $this->config = am_set_config($this->config);
            }
...
class Am_Recaptcha
{
    ...
    public function render($theme = null, $size = null, $moreVars = null)
    {
        ...
        return <<<CUT
        <script src="//www.google.com/recaptcha/api.js?hl=$locale" async defer></script>
...
class Am_App
{
    ...
    public function __construct($config)
    {
        ...
        defined('AM_HUGE_DB_NO_CACHE') ?: define('AM_HUGE_DB_NO_CACHE', false);
    ...
    public function bootstrap()
    {
        ...
        if (defined('AM_APPLICATION_ENV') && (AM_APPLICATION_ENV == 'debug')) {
            error_reporting(E_ALL | E_RECOVERABLE_ERROR | E_NOTICE | E_DEPRECATED);
        ...
        // Make sure configRefsCache is exists
        Am_Html_Refs::checkCache();
...
//            Zend_Translate::setCache($cache);
    ...
    function addRoutes(Am_Mvc_Router $router)
    {
        ...
        $router->addRoute('buy-preview', new Am_Mvc_Router_Route(
            'buy/preview',
            [
                'module' => 'default',
                'controller' => 'buy',
    ...
    public function initHooks()
    {
        ...
        $hook->add(Am_Event::HOURLY, [Am_Theme::class, 'doCacheCleanup']);
        ...
        $hook->add(Am_Event::HOURLY, Am_Laz::y('rateLimit',  'hourly'));
    ...
    function initConstants()
    {
        ...
        if (!defined('AM_VERSION'))
            define('AM_VERSION', '6.3.39');
        if (!defined('AM_BETA'))
            define('AM_BETA', '0' == 1);
        if (!defined('AM_HEAVY_MEMORY_LIMIT'))
            define('AM_HEAVY_MEMORY_LIMIT', '512M');
        if (!defined('AM_HEAVY_MAX_EXECUTION_TIME'))
            define('AM_HEAVY_MAX_EXECUTION_TIME', 60);

        if (!defined('AM_CC_RATE_LIMIT_HOURLY'))
            define('AM_CC_RATE_LIMIT_HOURLY', 16);

        if (!defined('AM_CC_RATE_LIMIT_DAILY'))
            define('AM_CC_RATE_LIMIT_DAILY', 32);

        if (!defined('AM_CC_RATE_LIMIT_MONTHLY'))
            define('AM_CC_RATE_LIMIT_MONTHLY', 64);
    }
    ...
    {
        if (
            $e instanceof Zend_Controller_Dispatcher_Exception
    ...
    {
        @[$found, ] = array_keys(Zend_Locale::getDefault());
        if (!$found)
            return 'en_US';
        if (!$addRegion)
            return $found;
    ...
    static function cleanupVendorDir()
    {
        ...
        {
            if (preg_match('#phing|composer|robo|phpstan|phpunit#i', $file->getPathname())) continue;
            if (preg_match('#/src/#', $file->getPathname())) continue;
            if ($file->isFile())
            {
                if (!preg_match('#\.php$|\.crt$|composer.json|installed.json|packages?.json#i', $file->getFilename()) ||
...
/** theme will return array of this elements to describe what to include into page HTML
 *  it is replacement for view->head..()->append.. code
 *  note: objects of this class MUST be serializable into cache and config
 **/
class Am_Html_Ref
{
    const T_CSS_URL = 'css';
    const T_CSS_INLINE = 'css_inline';
    const T_JS_URL = 'js';
    const T_JS_INLINE = 'js_inline';
    const T_CODE = 'code'; // plan HTML code
    ...
    /** @var array some additional attributes like "rel" "crossorigin" */
    protected array $attrs = [];

    private function __construct(string $id, string $type) { $this->id = $id; $this->type = $type; }

    static function cssUrl(string $id, string $url, array $attrs = []) : self {
        if (empty($attrs['rel'])) $attrs['rel'] = "stylesheet";
        $x = new self($id, self::T_CSS_URL); $x->url = $url; $x->attrs = $attrs;
        return $x;
    }
    static function jsUrl(string $id, string $url, array $attrs = []) : self {
        $x = new self($id, self::T_JS_URL); $x->url = $url; $x->attrs = $attrs;
        return $x;
    }
    static function cssInline(string $id, string $content) : self {
        $x = new self($id, self::T_CSS_INLINE); $x->content = $content; return $x;
    }
    static function jsInline(string $id, string $content, array $attrs = []) : self {
        if (empty($attrs['type'])) $attrs['type'] = "text/javascript";
        $x = new self($id, self::T_JS_INLINE); $x->content = $content;
        $x->attrs = $attrs;
        return $x;
    }
    static function code(string $id, string $content) : self {
        $x = new self($id, self::T_CODE); $x->content = $content; return $x;
    }

    function anon() : self {   $this->attrs['crossorigin'] = 'anonymous'; return $this;  }

    public function renderToView(Am_View $view, string $placeholder = self::R_HEAD) {
        switch ($this->type) {
            case Am_Html_Ref::T_CSS_URL:
                $view->headLink()->appendStylesheet([
                    'href' => $this->url,
                    'extras' => $this->attrs,
                ]);
                break;
            case Am_Html_Ref::T_CSS_INLINE:
                $view->headStyle()->appendStyle($this->content); break;
            case Am_Html_Ref::T_JS_URL:
                $view->headScript()->appendFile($this->url); break;
            case Am_Html_Ref::T_JS_INLINE:
                $view->headScript()->appendScript($this->content); break;
            case Am_Html_Ref::T_CODE:
                if ($placeholder === self::R_HEAD)
                    $view->placeholder('head-finish')->append($this->content);
                else
                    $view->placeholder('body-finish')->append($this->content);
            default:
                throw new Am_Exception_NotImplemented($this->type);
        }
    }

    /**
     * output array usable for JS include code
     * @return array
     */
    public function renderToJsInclude() : array {
        $ret = [
            $this->type,
            $this->url ?? $this->content,
        ];
        if ($this->attrs) $ret[] = $this->attrs;
        return $ret;
    }

    public function id() : string {  return $this->id;  }

    private function renderAtts(array $atts) : string {
        $ret = "";
        foreach ($atts as $k=>$v) {
            $ret .= Am_Html::escape($k) . '="' . Am_Html::escape($v) . '" ' ;
        }
        return $ret;
    }
    ...
    public function render() : string {
        switch ($this->type) {
            case Am_Html_Ref::T_CSS_URL:
                return sprintf('<link href="%s" %s>',
                    Am_Html::escape($this->url), $this->renderAtts($this->attrs),
                );
            case Am_Html_Ref::T_CSS_INLINE:
                return sprintf('<style media="screen">%s</style>',
                    $this->content
                );
            case Am_Html_Ref::T_JS_URL:
            ...
            case Am_Html_Ref::T_JS_INLINE:
                return sprintf('<script %s>%s</script>',
                    $this->renderAtts($this->attrs), $this->content
                );
            case Am_Html_Ref::T_CODE:
                return $this->content;
            default:
...
/* collection of CSS/JS includes that must be added into page <head> and <body> */
class Am_Html_Refs
{
    const CTX_VIEW = 'view'; // full page view
    const CTX_EMBED = 'embed'; // embedding amember elements to a page

    const CONFIG_REFS_EMBED = '_html_refs_embed';
    ...
    function __construct(string $ctx = self::CTX_VIEW) {
        $this->ctx = $ctx;
    }

    function headPrepend(Am_Html_Ref ...$refs) { $this->_prepend($this->head, ...$refs);  }
    function headAppend(Am_Html_Ref ...$refs)  { $this->_append($this->head, ...$refs);  }
    /** @param $ids string|string][ refId to find as insert point */
    function headBefore($ids, Am_Html_Ref ...$refs)  { $this->_before($this->head, $ids, ...$refs);  }
    /** @param $ids string|string][ refId to find as insert point */
    function headAfter($ids,  Am_Html_Ref ...$refs)  { $this->_after($this->head, $ids, ...$refs);  }
    /** @param $ids string|string][ refId to find as insert point */
    function headReplace($ids,  Am_Html_Ref ...$refs) : bool { return $this->_replace($this->head, $ids, ...$refs);  }
    function headRemove($ids) { return $this->_remove($this->head, $ids); }

    function bodyPrepend(Am_Html_Ref ...$refs) { $this->_prepend($this->body, ...$refs);  }
    function bodyAppend(Am_Html_Ref ...$refs)  { $this->_append($this->body, ...$refs);  }
    /** @param $ids string|string][ refId to find as insert point */
    function bodyBefore($ids, Am_Html_Ref ...$refs)  { $this->_before($this->body, $ids, ...$refs);  }
    /** @param $ids string|string][ refId to find as insert point */
    function bodyAfter($ids,  Am_Html_Ref ...$refs)  { $this->_after($this->body, $ids, ...$refs);  }
    /** @param $ids string|string][ refId to find as insert point */
    function bodyReplace($ids,  Am_Html_Ref ...$refs) : bool { return $this->_replace($this->body, $ids, ...$refs);  }
    function bodyRemove($ids) { return $this->_remove($this->body, $ids);  }

    /* return rendering context */
    public function context() : string { return $this->ctx;  }

    /** @return Am_Html_Ref[] */
    public function headRefs() : array {  return $this->head;  }
    /** @return Am_Html_Ref[] */
    public function bodyRefs() : array {  return $this->body;  }

    public function renderToView(Am_View $view) {
        foreach ($this->headRefs() as $ref) $ref->renderToView($view, 'head');
        foreach ($this->bodyRefs() as $ref) $ref->renderToView($view, 'body');
    }

    public function renderToJsInclude() : array {
        return [
            ...array_map(fn(Am_Html_Ref $r) => $r->renderToJsInclude(),  $this->headRefs()),
            ...array_map(fn(Am_Html_Ref $r) => $r->renderToJsInclude(),  $this->bodyRefs()),
        ];
    }

    public function renderHead() : string {
        return implode("\n", array_map(fn(Am_Html_Ref $r) => $r->render(),  $this->headRefs()));
    }

    public function renderBody() : string {
        return implode("\n", array_map(fn(Am_Html_Ref $r) => $r->render(),  $this->bodyRefs()));
    }

    /// internals
    protected function _prepend(array & $arr, Am_Html_Ref ...$args) { array_unshift($arr, ...$args); }
    protected function _append(array & $arr, Am_Html_Ref ...$args)  { array_push($arr, ...$args); }
    protected function _before(array & $arr, $ids, Am_Html_Ref ...$args) {
        if (!is_array($ids)) $ids = [$ids];
        $idx = 0;
        foreach ($arr as $k => $ref) if (in_array($ref->id(), $ids)) { $idx = $k; break;}
        array_splice($arr, $idx, 0, $args);
    }
    protected function _after(array & $arr, $ids, Am_Html_Ref ...$args) {
        if (!is_array($ids)) $ids = [$ids];
        $idx = count($arr);
        foreach ($arr as $k => $ref) if (in_array($ref->id(), $ids)) { $idx = $k; }
        array_splice($arr, $idx+1, 0, $args);
    }
    protected function _replace(array &$arr, $ids, Am_Html_Ref ...$args) : bool {
        $ret = false;
        if (!is_array($ids)) $ids = [$ids];

        foreach ($ids as $id) {
            foreach ($arr as $k => $ref) { if ( $ref->id() === $id ) { // $k is shifted after splice
                array_splice($arr, $k, 1, $args );
                $args = []; // we insert only once even for multiple ids
                $ret = true;
            }
        }}
        return $ret;
    }
    protected function _remove(array & $arr, $ids) {
        if (!is_array($ids)) $ids = [$ids];
    ...
    public function getIds() {
        return [
            'head' => array_map( fn(Am_Html_Ref $r) => $r->id(), $this->head),
            'body' => array_map( fn(Am_Html_Ref $r) => $r->id(), $this->body),
        ];
    }

    /**
     * Run necessary hooks to fill current object with values from theme, frontend, hooks, etc
     * @param Am_Di $di
     * @param Am_View $view
     * @return void
     */
    public function fillUser(Am_Di $di, $view)
    {
        $view->theme->htmlRefs($this, $view);

        if ($siteCss = $view->_scriptCss('site.css')) {
            $_ = str_replace(parse_url(ROOT_URL, PHP_URL_PATH), '', $siteCss);
            $st = "?" . crc32(filemtime(ROOT_DIR . $_));
            $this->headAppend(Am_Html_Ref::cssUrl('site-css', $siteCss . $st ));
        }
        if ($this->context() === self::CTX_VIEW) {
            $t = "?" . AM_VERSION_HASH;
            $this->headAppend(Am_Html_Ref::cssUrl('jquery-ui-css', $view->_scriptJs('jquery/jquery.ui.css').$t));
        }

        $di->hook->call(Am_Event::HTML_REFS, [ 'htmlRefs' => $this, 'view' => $view, ]);
        // this will be extended in v7
    }

    /* called when theme settings changed */
    public static function updateCache()
    {
        $di = Am_Di::getInstance();
        // build new cache
        $view = $di->view;
        $htmlRefs = new Am_Html_Refs(Am_Html_Refs::CTX_EMBED);
        $htmlRefs->fillUser($di, $view);
        $jsIncludes = $htmlRefs->renderToJsInclude();
        $newv = [ $jsIncludes, $view->theme->getId(), $di->time, ] ;
        Am_Config::saveValue(self::CONFIG_REFS_EMBED, serialize($newv));
    }

    /** this function check and write config theme includes cache. the cache may be used by lite without including any am code */
    static function checkCache(bool $force = false)
    {
        $di = Am_Di::getInstance();
        $theme = $di->theme;
        if (!$force) {
            $cv = $di->config->get(self::CONFIG_REFS_EMBED);
            if ($cv && ($cv = unserialize($cv))) {
                if ($cv[0] && ($cv[1] === $theme->getId())) return; // we have correct cache
            }
        }
...
/* html color manipulations */
class Am_Color
{
    static function inverse(string $color) : string
    {
    ...
    static function brightness(string $color, int $steps) : string
    {
...
class Am_Cookie
{
    //date_default_timezone_set('UTC'); mktime(23, 59, 59, 12, 31, 9999);
    ...
    {
        $expires = min($expires, self::MAX_EXPIRES);

        if (self::$_unitTestEnabled) {
            self::$_cookies[$name] = $value;
        } else {
            setcookie($name, $value, $expires, $path,
                ($strictDomainName ? $domain : self::_getCookieDomain($domain)), $secure, $httponly);
        }
    }
        ...
        if (self::$_unitTestEnabled) {
            ...
            if (
                ...
                if (version_compare(phpversion(), '7.3.0') >= 0) {
                    ...
                    setcookie($name, $value, $expires, $path . '; samesite=none',
                        ($strictDomainName ? $domain : self::_getCookieDomain($domain)), $secure, $httponly);
                }
            } else {
                setcookie($name, $value, $expires, $path,
                    ($strictDomainName ? $domain : self::_getCookieDomain($domain)), false, $httponly);
            }
        }
    }
...
class Am_View_Sprite
{
    protected static $_sprite_offsets = [
        'icon' => [
            ...
            'clear-cache' => 37772,
            'wordpress-sync' => 38038,
            'subusers-invite-send' => 38304,
            'oauth-clients' => 38570,
            'oauth-tokens' => 38836,
            'two-factor-email' => 39102,
        ],
        'flag' => [
            'ad' => 26,
            'ae' => 52,
    ...
    {
        return self::$_sprite_offsets[$source][$id] ?? false;
    }
}

...
/**
 * View helper to display menu
 * @package Am_View
 */
class Am_View_Helper_Menu extends Am_View_Helper_NavigationAbstract
{
    protected $_normalClass = "";
    protected $_disabledClass = "disabled";
    protected $_hasChildrenClass = 'has-children';
    ...
    /**
     * Add this class to a/span elements
     * @var null
     */
     ...
     *                                               operate on
     * @return Am_View_Helper_Navigation_Menu      fluent interface,
     *                                               returns self
     */
    public function menu(?Am_Navigation_Container $container = null)
    {
        if (null !== $container) {
    ...
    public function htmlify(Am_Navigation_Page $page)
    {
        ...
        if ($this->_linkClass)
            $attribs['class'] = (empty($attribs['class']) ? '' : $attribs['class']) . ' ' . $this->_linkClass;
    ...
    protected function _normalizeOptions(array $options = [])
    {
        ...
        foreach (['activeClass', 'normalClass', 'disabledClass', 'linkClass'] as $k)
            if (!empty($options[$k]))
                $this->{'_'.$k} = $options[$k];
     ...
     */
    public function renderMenu(Am_Navigation_Container $container,
                               array $options = []
    )
    {
        $this->_container = $container;

        $options = Am_Di::getInstance()->hook->filter($options, Am_Event::MENU_BEFORE_RENDER, [
            'container' => $container,
        ]);
        ...
        $html = Am_Di::getInstance()->hook->filter($html, Am_Event::MENU_AFTER_RENDER, [
            'container' => $container,
            'options' => $options,
        ]);
    ...
    function getLiClass(Am_Navigation_Page $subPage, $isActive)
    {
        ...
        if ($liClass = $subPage->get('li-class'))
            $class .= $liClass . ' ';
        if ($class = trim($class))
            return " class=\"$class\"";
    }
                                   ...
                                   $renderParentClass)
    {
        ...
        foreach ($iterator as $page) {
            ...
            if ($depth > $prevDepth) {
                ...
                $html .= $myIndent .  '<ul' . $ulClass . $ulId . '>' . self::EOL;

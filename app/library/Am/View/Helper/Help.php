    ...
    function __invoke($id)
    {
        ...
        if (!is_string($id))
        {
            if ($id instanceof Am_Plugin_Base)
            {
                ...
                if($plId != $id->getId()) {
                    $params = [
                        'https://example.com/amember/' . strtolower($type) . '/' . $plId =>
                            $params['root_surl'] . '/' . strtolower($type) . '/' . $id->getId()] + $params;
                    }
...
CUT;
    ...
    static function helpUrl($topic)
    {
        ...
        if (defined('AM_LOCAL_DEV_DOCS') && AM_LOCAL_DEV_DOCS)
            ...
            return 'https://docs.amember.com/' . $topic . (AM_VERSION !== '6.3.39' ? '?v='.AM_VERSION : '');
    }

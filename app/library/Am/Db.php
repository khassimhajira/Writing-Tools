...
class Am_Db extends DbSimple_Mypdo
{
    ...
    protected function _performQuery($queryMain, $resultOnly = false)
    {
        if ($this->PDO instanceof Am_Db_PDOProxy)
            $this->PDO->_setup(); // magic call to convert and do not in middle of another query!
        return parent::_performQuery($queryMain, $resultOnly);
    }
    ...
    {
        if (AM_HUGE_DB && !AM_HUGE_DB_NO_CACHE && $this->_cacheNext) {

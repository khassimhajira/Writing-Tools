...
class Am_ftp_base {
	...
	function __construct($port_mode=FALSE, $verb=FALSE, $le=FALSE) {
		$this->LocalEcho=false; // to make sure it does not echo to server error log $le;
		$this->Verbose=$verb;
...
// <!-- --------------------------------------------------------------------------------------- -->
	...
	function SendMSG($message = "", $crlf=true) {
		if ($this->Verbose) {
//          disable as it writes to php logs on server
...
// Rcupre une erreur externe
     ...
     * @return ftp_base
     */
    static function create($port_mode=FALSE, $verb=FALSE, $le=FALSE)
    {
        $mod_sockets = extension_loaded( 'sockets' );
        $class = $mod_sockets ? 'Am_ftp_sockets' : 'Am_ftp_pure';
        class_exists($class, true);
        $rc = new ReflectionClass($class);
        return $rc->newInstance(func_get_args());
    }

    // added by alex@cgi-central.ne
    // logging

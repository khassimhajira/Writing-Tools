...
<p>According to new EU VAT rules you are required to collect two pieces of non-conflicting evidence of customer's location country if you are selling Digital (Electronic Service) Products. These two pieces of evidence will be checked on each invoice which has Digital Product included:</p>
<ul class="list">
...
EOT
        ...
        $fieldSet->addAdvCheckbox('tax_location_validation')
            ->setLabel(___("Enable Location Validation\n" .
                ...
                'Invoice that fails validation will be blocked and user will receive warning.'));
        ...
        $fieldSet->addSelect('my_country')
            ...
            ->loadOptions(['' => ___('-- Outside of EU VAT Zone')] + $this->countries);

        $fieldSet->addText('my_id')
            ->setLabel(___('VAT Id'))
            ->addRule('required', 'this field is required', null, HTML_Quickform2_Rule::ONBLUR_CLIENT);

        $fieldSet->addScript()
            ->setScript(<<<CUT
jQuery(function () {
    jQuery('[name$="my_country"]').change(function () {
        jQuery('[name$="my_id"]').closest('.am-row').toggle(jQuery(this).val() !== '')
        if (jQuery(this).val() == '') {
            jQuery('[name$="my_id"]').val('')
        }
    }).change()
})
CUT
...
<style>
    ...
    .tax-rate-container .fieldset .am-row .am-element-title,
    .tax-rate-container .fieldset .am-row .am-element {
        padding-bottom: 0;
    }
    ...
    .tax-rate-container.tax-rate-container-add .fieldset .am-row:nth-child(2) {
        width:auto;
        float:none;
    }
    
    .tax-rate-container.tax-rate-container-standard .fieldset .am-row:first-child {
        width:auto;
        float:none;
    }
...
jQuery(function($){
    ...
    jQuery(document).on('keyup change', '.am-input-filter', function () {
        let context = jQuery(this).closest('.am-input-filter-wrapper')
        jQuery('.am-input-filter-empty', context).toggle(jQuery(this).val().length !== 0)

        let scope = context.data('search-scope')

        if (jQuery(this).val()) {
            jQuery('[data-search-target=' + scope + ']').closest('.am-row').hide()
            jQuery('[data-search-target=' + scope + '][data-title*="' + jQuery(this).val().toLowerCase() + '"]').closest('.am-row').show()
        } else {
            jQuery('[data-search-target=' + scope + ']').closest('.am-row').show()
        }
    })

    jQuery(document).on('click', '.am-input-filter-empty', function () {
...
jQuery(function($){
    ...
    function toggleState()
    {
        $('.tax-rate').each(function(){
            let flagHas = hasSchedule($(this).data('id'), $(this).data('country'))

            $(this).closest('.am-row').toggleClass('tax-rate-has-rule', flagHas)
            if (flagHas) {
                let d = getSchedule($(this).data('id'), $(this).data('country'))
                $(this).
                    closest('.am-row').find('.tax-rate-schedule-preview-date').
                    text(d.date)
                $(this).
                    closest('.am-row').find('.tax-rate-schedule-preview-rate').
                    text(d.rate)
            }
        })
    ...
    let taxSchedule = jQuery('#tax-rate-schedule-data').val() ? JSON.parse(jQuery('#tax-rate-schedule-data').val()) : {}
    ...
    $('.tax-rate').each(function(){
        ...
        let i = 1
        let html = '{$i18n['change_to']}'.replace(/%s/g, function(e) {return s.shift();}, 'g')
    ...
    jQuery("#tax-rate-schedule").dialog({
        buttons: {
            Save: function(){
                let d = _GetSQLDate(jQuery(this).find('[name=date]').datepicker('getDate'))
                let r = jQuery(this).find('[name=rate]').val()
...
CUT
    ...
    function addRatesTable($form, $id, $title, $addDeleteLink = false)
    {
        $fs = $form->addAdvFieldset($id, [
            ...
            'class' => 'tax-rate-container ' . ($addDeleteLink ? 'tax-rate-container-add' : 'tax-rate-container-standard')
        ])
            ->setLabel($title);
        if ($addDeleteLink) {
...
CUT
        ...
        $fs->addHtml(null, ['class' => 'am-no-label'])
            ->setHtml(<<<CUT
    <div class="am-input-filter-wrapper" data-search-scope="$id">
...
CUT
        ...
        foreach ($this->rates as $c => $rate) {
            ...
            $r = $fs->addText("$id.$c", [
                ...
                'data-title' => strtolower($this->countries[$c]),
    ...
    public function getRate(Invoice $invoice, ?InvoiceItem $item = null)
    {
        ...
        $country = $this->hasDigitalProducts($invoice) ? ($id ? substr($id, 0, 2) : ( is_null($u) ? false : $u->get('country'))) : $this->getConfig('my_country');
    ...
    protected function _getRatePerProduct($id, ?InvoiceItem $item = null)
    {
        ...
        if (!empty($item) && $item->item_type == 'product') {
            if (($product = $item->tryLoadProduct()) && $product->tax_rate_group) {
                ...
                $customRate = (is_array($rates) && !empty($rates[$id])) ? $rates[$id] : null;
            }
        }
    ...
    function onInvoiceValidate(Am_Event $event)
    {
        ...
        if (
            ...
            if (!$this->locationIsValid($invoice))
                $event->setReturn(___("Location validation failed.")

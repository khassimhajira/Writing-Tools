...
abstract class Am_Protect_Databased extends Am_Protect_Abstract implements Am_Protect_SingleLogin
{
    ...
    function configCheckDbSettings(array $config)
    {
        ...
        try
        {
            ...
            $fields = implode(',', $this->guessFieldsPattern);
    ...
    {
        return array_filter(array_map('trim', $this->getConfig('admin_groups', [])));
    }

    function getBannedGroups()
    {
        return array_filter(array_map('trim', $this->getConfig('banned_groups', [])));
    }
    ...
    function chooseGroups($groups, ?User $user = null)
    {
        ...
        if($user && $user->isLocked() && ($locked = $this->getLockedGroup()))
            return ($this->groupMode == self::GROUP_SINGLE ? $locked : [$locked]);
    ...
    {
        $this->_priority = array_flip($this->getConfig('priority', []));
    }
    ...
    function getDb()
    {
        if (!$this->_db)
        {
            ...
            $this->_db->setErrorHandler([$this, 'dbErrorHandler']);
    ...
    function onSubscriptionChanged(Am_Event_SubscriptionChanged $event, ?User $oldUser = null)
    {
        try {
            ...
            if ($found)
            {
                if($this->canUpdate($found)){
                    $this->getTable()->updateFromAmember($found, $user, $this->calculateGroups($oldUser, true));
        ...
        {
            $this->getDi()->logger->error("Error changing subscriptions " . $this->getId(), ["exception" => $e]);
        }
    }

    function onUserAfterUpdate(Am_Event_UserAfterUpdate $event)
    {
        $e = new Am_Event_SubscriptionChanged($event->getUser(), [], []);
        $oldUser = $event->getOldUser();
        $user = $event->getUser();
        $_is_locked = $oldUser->is_locked;
        $oldUser->is_locked = $user->is_locked;
        $_ = $this->onSubscriptionChanged($e, $oldUser);
        $oldUser->is_locked = $_is_locked;
        return $_;
    }
    ...
    public function onAuthTryLogin(Am_Event_AuthTryLogin $event)
    {
        ...
        if ($p_b = $this->getConfig('auto_create_billing_plan'))
        {
            ...
            {
                //just log
                $this->getDi()->logger->error("Exception onAuthTryLogin {plugin}", ["exception" => $e, 'plugin' => $this]);
            };
        }
    ...
    function getAvailableUserGroups()
    {
        ...
        foreach ($this->getDb()->select($this->getAvailableUserGroupsSql()) as $r)
        {
            try {
                ...
                $this->getDi()->logger->error("Error while getting available user groups in " . $this->getId(), ["exception" => $e]);
                throw $e;
            }
            $ret[$g->getId()] = $g;
        }
    ...
    function getManagedUserGroups()
    {
        ...
        foreach ($groups as $k => $group)
            if (in_array($group->getId(), $this->getConfig('admin_groups', [])) || in_array($group->getId(), $this->getConfig('banned_groups', [])))
                unset($groups[$k]);
        return $groups;
    }
        ...
        switch (intval($step))
        {
            ...
            case 1:
                ...
                foreach ($records as $user)
                {
                    $count++;
                    /* @var $user User */
                    $this->onSubscriptionChanged(new Am_Event_SubscriptionChanged($user, [], []));
                }
                if (!$count)
                {
                    $context = null;

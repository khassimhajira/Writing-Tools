...
class Am_Report_PaymentBySignupForm extends Am_Report_Date
{
    ...
    public function getQuery()
    {
        ...
        foreach ($this->getForms() as $k => $title) {
            $form = $q->escape($k);
            $q->addField("ROUND(SUM(IF(i.saved_form_id=$form, i.first_total/i.base_currency_multi, 0)),2)\n", 'amt_' . $k);
        }
        return $q;
    }
...
class Am_Report_PaymentCountry extends Am_Report_Date
{
    ...
    public function getQuery()
    {
        ...
        $q->leftJoin('?_user', 'u', 'p.user_id=u.user_id');
        foreach ($this->getCountries() as $country) {
            $escaped_country = $this->getDi()->db->escape($country);
            $q->addField("ROUND(SUM(IF(IFNULL(country, '')=$escaped_country, amount/base_currency_multi, 0)), 2)", 'amt_' . md5($country));
        }
    ...
    function getLines()
    {
        ...
        foreach ($this->getCountries() as $country) {
            ...
            $ret[] = new Am_Report_Line('amt_' . md5($country),
...
class Am_Report_PaymentCity extends Am_Report_Date
{
    public function __construct()
    {
        $this->title = ___('Payments by Customer City');
    }
    ...
    public function getQuery()
    {
        ...
        foreach ($this->getCities() as $city) {
            $escaped_city = $this->getDi()->db->escape($city);
            $q->addField("ROUND(SUM(IF(IFNULL(city, '')=$escaped_city, amount/base_currency_multi, 0)), 2)", 'amt_' . md5($city));
        }
    ...
    function getLines()
    {
        $ret = [];
        foreach ($this->getCities() as $city) {
            $title = $city ?: ___('Unknown');
            $ret[] = new Am_Report_Line('amt_' . md5($city),
                sprintf('%s, %s', $title, Am_Currency::getDefault()),
                null, ['Am_Currency', 'render']);
        }
        return $ret;
    ...
    {
        parent::_initConfigForm($form);
        $opt = $this->getAvailableCities();
        if (!$opt) {
            $text = Am_Html::escape(___('You have not any payments associated with cities yet.'));
            $form->addHtml(null, ['class' => 'am-row-wide'])
                ->setHtml(<<<CUT
<span class="red" id="r-payment-city">$text</span>
<script type="text/javascript">
jQuery(function(){
    jQuery('#r-payment-city').closest('.am-row').nextAll('.am-row').remove();
});
</script>
CUT
                );
        } else {
            $form->addMagicSelect('city', ['class' => 'am-combobox-fixed'])
                ->setLabel(___("Cities\nkeep empty to report all"))
                ->loadOptions($opt);
        }
    }
    ...
    {
        $vars = $this->form->getValue();
        return $vars['city'] ?: $this->getAllCities();
    }

    protected function getAvailableCities()
    {
        return $this->getDi()->db->selectCol(<<<CUT
            SELECT DISTINCT city, city AS ARRAY_KEY
                FROM ?_user
                LEFT JOIN ?_invoice_payment USING (user_id)
...
class Am_Report_PurchaseCount extends Am_Report_Date
{
    ...
    function getProducts()
    {
        ...
        $cache = $this->getDi()->db->selectCol("SELECT
            ...
            FROM ?_product
            {WHERE product_id IN (?a)}
            ORDER BY sort_order, title", $vars['products'] ?: DBSIMPLE_SKIP);
        return $cache;
    }
...
class Am_Report_BillingPlanPurchaseCount extends Am_Report_Date
{
    public function __construct()
    {
        $this->title = ___('Count of billing plan purchase');
    }

    public function _initConfigForm(Am_Form $form)
    {
        parent::_initConfigForm($form);
        $form->addMagicSelect('bp_ids', ['class' => 'am-combobox-fixed'])
            ->setLabel(___("Billing Plans\nkeep empty to report all"))
    ...
    public function getQuery()
    {
        ...
        foreach ($billingPlans as $k => $v) {
            $q->addField("SUM(IF(ii.billing_plan_id = $k, ii.qty, 0))", 'cnt_' . $k);
        }
        $q->addField("SUM(IF(ii.billing_plan_id IN (".implode(', ', array_map(function($p){return $p;}, array_keys($billingPlans)))."), ii.qty, 0))", 'cnt_total');


        return $q;
    }

    function getBillingPlans(): array
    {
        $vars = $this->form->getValue();
        $options = $this->getDi()->billingPlanTable->getOptions();

        if (empty($vars['bp_ids'])) return $options;

        $options = array_filter($options, fn($bp_id) => in_array($bp_id, $vars['bp_ids']), ARRAY_FILTER_USE_KEY);
    ...
    {
        $ret = [];
        foreach ($this->getBillingPlans() as $k => $title) {
            $ret[] = new Am_Report_Line('cnt_' . $k, ucfirst($title));
        }
        $ret[] = new Am_Report_Line('cnt_total', 'Total');
        return $ret;
...
class Am_Report_Downloads extends Am_Report_Date
{
    ...
    {
        return $this->getDi()->db->selectCol("SELECT DISTINCT file_id AS ARRAY_KEY, title FROM ?_file");
    }
    ...
    function getFiles()
    {
        ...
        $files = $this->getDi()->db->selectCol("SELECT
                ...
                FROM ?_file
                {WHERE file_id IN (?a)}", $vars['files'] ?: DBSIMPLE_SKIP);
        return $files;
    }
...
CUT
        ...
        $this->getDi()->db->query(<<<CUT
            INSERT IGNORE INTO ?_active_users_by_period_tmp (point)
                ...
                UNION
                SELECT DISTINCT($expend) FROM ?_access WHERE expire_date BETWEEN ? AND ?
CUT
            , $this->start, $this->stop, $this->start, $this->stop);
        ...
        $this->getDi()->db->query(<<<CUT
            UPDATE ?_active_users_by_period_tmp p
                SET p.cnt = (SELECT COUNT(DISTINCT user_id) FROM ?_access a WHERE p.point BETWEEN $expbegin AND $expend {AND a.product_id IN (?a)})
CUT
            , $vars['products'] ?: DBSIMPLE_SKIP);
    ...
    public function getLines()
    {
        return [new Am_Report_Line('cnt', ___('Active Users'))];
    }
}
    ...
    public function getQuery()
    {
        ...
        if (!empty($field->sql)) {
            foreach ($field->options as $k => $v) {
                $fname = $q->escape($field->name, true);
                $kk = $q->escape($k);
                $q->addField("SUM(IF(IFNULL($fname, '')=$kk, 1, 0))", 'cnt_' . sha1($k));
            }
        } else {
            $fname = $q->escape($field->name);
            $q->leftJoin('?_data', 'd', "d.`table`='user' AND d.`id`=u.user_id AND d.`key`=$fname");
            foreach ($field->options as $k => $v) {
                $kk = $q->escape($k);
                $q->addField("SUM(IF(IFNULL(d.`value`, '')=$kk, 1, 0))", 'cnt_' . sha1($k));
            }
        }
        return $q;
    }
...
class Am_Report_PaymentNumByBillingPlan extends Am_Report_Date
{
    public function __construct()
    {
        $this->title = ___('Number of Payments by Billing Plan');
    }
    ...
    public function getQuery()
    {
        $q = new Am_Query($this->getDi()->invoicePaymentTable, 'p');
        $q->leftJoin('?_invoice', 'i', 'i.invoice_id=p.invoice_id');
        $q->leftJoin('?_invoice_item', 'ii', 'ii.invoice_id=i.invoice_id');
        $q->clearFields();

        $billingPlans = $this->getBillingPlans();
        foreach ($billingPlans as $k => $v) {
            $q->addField("SUM(IF(ii.billing_plan_id = $k, IF(DATE(p.dattm) = DATE(i.tm_started) OR ii.rebill_times>0, 1, 0), 0))", 'cnt_' . $k);
        }
    ...
    {
        $vars = $this->form->getValue();
        $products = $this->getDi()->productTable->selectObjects(<<<CUT
            SELECT *
                FROM ?_product
                {WHERE product_id IN (?a)}
                ORDER BY sort_order, title
            CUT, $vars['products'] ?: DBSIMPLE_SKIP);

        $out = [];
        /** @var Product $product */
        foreach ($products as $product) {
            $out += $product->getBillingOptions();
        }

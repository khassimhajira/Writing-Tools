...
class SendpassController extends Am_Mvc_Controller
{
    ...
    public function doSend()
    {
        ...
        do {
            ...
            if ($error = $this->getDi()->hook->filter('', Am_Event::AUTH_LOST_PASS_USER_CHECK, ['user' => $user])) {
                $title = ___('Lost Password Sending Error');
                $text = $error;
                $ok = false;
                break;
...
CUT;
    ...
    protected function createForm(User $user)
    {
        $form = new Am_Form();
        ...
        $pass0->addRule('callback2','wrong', fn($vars, $el) => $this->validateNewPass($vars, $el, $user));
    ...
    public function validateNewPass($vars, HTML_QuickForm2_Element_InputPassword $el, $user)
    {
        $vars = $el->getContainer()->getValue();
        if ($vars['pass0'] != '') {
            if ($user->checkPassword($vars['pass0'])) {
                return ___('Your new password must be different from your current password');
            }
        }
    }

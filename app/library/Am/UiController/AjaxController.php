...
class AjaxController extends Am_Mvc_Controller
{
    ...
    function ajaxCheckUniqLogin($vars)
    {
        ...
        return $this->_response->ajaxResponse($msg ?: true);
    }
...
/*
         if(!$this->getDi()->couponTable->validateCode($vars['coupon'])){
            return $this->_response->ajaxResponse(___('Invalid coupon code'));
        } */
    ...
    function invoiceSummaryAction()
    {
        ...
        if (!empty($vars['_button']) && ($btn = $this->getDi()->buttonTable->findFirstByHash($vars['_button']))) {
            ...
            $invoice->calculate();
            if (
                ($invoice->first_total > 0 || $invoice->second_total > 0)
    ...
    function unsubscribedAction()
    {
        ...
        if ($user->unsubscribed != $v) {
            ...
            $this->getDi()->hook->call(Am_Event::USER_UNSUBSCRIBED_CHANGED, ['user' => $user, 'unsubscribed' => $v]);
        }

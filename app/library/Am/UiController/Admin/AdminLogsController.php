...
class AdminLogsController extends Am_Mvc_Controller_Pages
{
    ...
    public function initPages()
    {
        ...
        if ($admin->hasPermission(Am_Auth_Admin::PERM_LOGS_CRON))
    ...
    public function createErrors($id, $title, $controller)
    {
        ...
        $q->addField("(SELECT CONCAT(m.name_f, ' ', m.name_l) FROM ?_user m WHERE m.user_id=t.user_id)", 'member_name');
        $q->addField("(SELECT m.login FROM ?_user m WHERE m.user_id=t.user_id)", 'member_login');
    ...
    {
        $q = new Am_Query($this->getDi()->accessLogTable);
        $q->addField("(SELECT CONCAT(m.name_f, ' ', m.name_l) FROM ?_user m WHERE m.user_id=t.user_id)", 'member_name');
        $q->addField("(SELECT m.login FROM ?_user m WHERE m.user_id=t.user_id)", 'member_login');
        $q->setOrder('time', true);

        $g = new Am_Grid_Editable('_log_access', ___('Access Log'), $q, $this->getRequest(), $this->view);
        $g->setPermissionId(Am_Auth_Admin::PERM_LOGS_ACCESS);
        $g->actionsClear();
        $g->addField(new Am_Grid_Field_Date('time', ___('Date/Time')));
        $g->addField(new Am_Grid_Field('member_login', ___('User'), true, '', [$this, 'renderMember']));
        $g->addField(new Am_Grid_Field('method', ___('Method'), true, null, null, '1%'));
...
CUT
    ...
    public function createCronLog()
    {
        ...
        $grid->setPermissionId(Am_Auth_Admin::PERM_LOGS_CRON);

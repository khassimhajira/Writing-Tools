...
{
    protected bool $isInvoiceJoin = false;
        ...
        if (@$filter['text']) {
            switch (@$filter['type']) {
                case 'invoice':
                    ...
                    break;
                case 'invoice_id':
                    $q->addWhere('t.invoice_id=?', $filter['text']);
                    break;
                case 'invoice_ids':
                    $q->addWhere('t.invoice_id IN (?a)', explode(',', $filter['text']));
                    break;
                case 'login':
                    $q->addWhere('m.login=?', $filter['text']);
                    break;
                case 'aff':
                    if ($q->getTableName() !== '?_invoice' && !$this->isInvoiceJoin) {
                ...
                case 'name':
                    ...
                    break;
                case 'country':
                    $q->leftJoin('?_country', 'c', "c.country=m.country");
                    $q->addWhere("c.title LIKE ?", '%' . $filter['text'] . '%');
                    break;
                case 'receipt':
                    if ($q->getTableName() == '?_invoice') {
                        $q->leftJoin('?_invoice_payment', 'p');
                        $q->addWhere('p.receipt_id LIKE ?', '%' . $filter['text'] . '%');
                    } else {
                        $q->addWhere('t.receipt_id LIKE ?', '%' . $filter['text'] . '%');
                    }
                    break;
                case 'coupon':
                    if ($q->getTableName() !== '?_invoice' && !$this->isInvoiceJoin) {
                    ...
                    $alias = $q->getTableName() == '?_invoice' ? 't' : 'i';
                    $q->addWhere("$alias.coupon_code=?", $filter['text']);
                    break;
                case 'payment_ids' :
                    if ($q->getTableName() == '?_invoice') {
                        $q->leftJoin('?_invoice_payment', 'p');
                        $q->addWhere('p.invoice_payment_id IN (?a)', explode(",", $filter['text']));
                    } else {
                        $q->addWhere('t.invoice_payment_id IN (?a)', explode(",", $filter['text']));
                    }
                    break;
            }
        }
    ...
    public function renderInputs()
    {
        ...
        foreach (Am_Di::getInstance()->billingPlanTable->getOptions() as $id => $title) {
            $bpOptions["bp$id"] = $title;
        }

        $pOptions = [];
        $pOptions = $pOptions
            + Am_Di::getInstance()->productTable->getProductOptions()
            + [___('Billing Plans') => $bpOptions];
        ...
        $paysysOptions = Am_Html::renderOptions($paysysOptions, @$filter['paysys_id']);

        $options = Am_Html::renderOptions([
                    'invoice' => ___('Invoice Number'),
                    'invoice_id' => ___('Invoice ID'),
                    'invoice_ids' => ___('Invoice IDs (CSV)'),
...
class Am_Grid_Filter_Payments extends Am_Grid_Filter_Payments_Abstract
{
    ...
    protected function applyFilter()
    {
        ...
        if (!empty($filter['ptype'])) {
            switch ($filter['ptype']) {
                case 'sale':
                    $q->addWhere('t.is_first = 1');
                    break;
                case 'rebill':
                    $q->addWhere('t.is_first = 0');
                    break;
                case 'recurring':
            ...
            $q->addWhere(<<<CUT
                        EXISTS (SELECT invoice_item_id
                            ...
                            WHERE ii.invoice_id=t.invoice_id
                                ...
                                AND (t.is_first = 1 OR $alias.second_total > 0)
                        )
CUT
                , "%{$filter['text']}%");
        }
        ...
        if (!empty($filter['product_id'])) {
            $pids = [];
            $bpids = [];
            foreach ($filter['product_id'] as $id) {
                if (preg_match('/^bp([0-9]+)$/', $id, $_)) {
                    $bpids[] = $_[1];
                } else {
                    $pids[] = $id;
                }
            ...
            $pids = $this->grid->getDi()->productTable->extractProductIds($pids);
            $pids[] = -1;
            $bpids[] = -1;
...
class Am_Grid_Filter_Refunds extends Am_Grid_Filter_Payments_Abstract
{
    ...
    protected function applyFilter()
    {
        // Invoice table is already joined as 'i' in createRefundsPage()
        $this->isInvoiceJoin = true;
        parent::applyFilter();
        $filter = (array) $this->vars['filter'];
        $q = $this->grid->getDataSource();
        ...
        if (!empty($filter['text']) && $filter['type'] == 'items') {
            // Invoice table already joined, no need to join again
            $q->addWhere(<<<CUT
                        EXISTS (SELECT invoice_item_id
                            FROM ?_invoice_item ii
                            WHERE ii.invoice_id=t.invoice_id
                                AND ii.item_title LIKE ?
                                AND (ip.is_first = 1 OR i.second_total > 0)
                        )
CUT
                , "%{$filter['text']}%");
        }
...
class AdminPaymentsController extends Am_Mvc_Controller_Pages
{
    ...
    function createPaymentsPage()
    {
        ...
        $query->leftJoin('?_user', 'm', 'm.user_id=t.user_id')
            ->addField("(SELECT GROUP_CONCAT(IF(qty > 1, CONCAT(item_title, ' (', qty, ' pcs)' ), item_title)  SEPARATOR ', ') FROM ?_invoice_item WHERE invoice_id=t.invoice_id AND (t.is_first = 1 OR second_total > 0))", 'items')
        ...
        $grid->addField(new Am_Grid_Field_Expandable('refund_amount', ___('Refunded')))
            ->setPlaceholder(function($amt, $r){
                return sprintf('<div class="red" style="text-align:right; white-space: nowrap">&minus;%s</div>', Am_Currency::render($amt, $r->currency));
            })
        ...
        $invoiceField = $grid->addField('_invoice_id', ___('Invoice (Internal Id/Public Id)'))
            ->setGetFunction([$this, '_getInvoiceNum']);
    ...
    function createRefundsPage()
    {
        ...
        $query->leftJoin('?_user', 'm', 'm.user_id=t.user_id')
            ...
            ->leftJoin('?_invoice', 'i', 't.invoice_id=i.invoice_id')
            ->addField("(SELECT GROUP_CONCAT(IF(qty > 1, CONCAT(item_title, ' (', qty, ' pcs)' ), item_title)  SEPARATOR ', ') FROM ?_invoice_item WHERE invoice_id=t.invoice_id AND (ip.is_first = 1 OR i.second_total > 0))", 'items')
            ->addField('m.login', 'login')
            ->addField('m.email', 'email')
            ->addField('m.street', 'street')
            ->addField('m.street2', 'street2')
    ...
    function createInvoicesPage($page)
    {
        ...
        $action->addField(new Am_Grid_Field('tm_added', ___('Added (Date/Time)')))
            ...
            ->addField(new Am_Grid_Field('second_total', ___('Second Total')))
            ->addField(new Am_Grid_Field('second_tax', ___('Second Tax')))
            ->addField(new Am_Grid_Field('currency', ___('Currency')))
    ...
    function invoiceCardAction()
    {
        ...
        $this->view->invoice = $invoice = $this->getDi()->invoiceTable->load($this->getParam('id'));

        $upgrade_from = null;
        if ($invoice->tm_started && ($upgrade_from_invoice_id = $invoice->data()->get('upgrade-invoice_id'))) {
            $upgrade_from = $this->getDi()->invoiceTable->load($upgrade_from_invoice_id);
        }

        $upgrade_to = null;
        if (empty($upgrade_from)) {
            $upgrade_to = $this->getDi()->invoiceTable->findFirstByData('upgrade-invoice_id', $invoice->pk());
        }

        $this->view->upgrade_from = $upgrade_from;
        $this->view->upgrade_to = $upgrade_to;

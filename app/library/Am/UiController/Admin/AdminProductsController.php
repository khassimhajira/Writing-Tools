...
CUT
        ...
        foreach ($plans as $plan) {
            $fieldSet = $this->addFieldset('', ['id' => 'plan-' . $plan['plan_id'], 'class' => 'billing-plan'])
                    ->setLabel('<span class="plan-title-text">' . ($plan['title'] ?: "title - click to edit") . '</span>' .
        ...
        $this->addScript()
            ->setScript(<<<CUT
jQuery(function(){
    let tabs = jQuery('#billing-plan-wrap');
    tabs.tabs();
    tabs.tabs('disable', -1);
...
CUT
    ...
    function addBillingPlanElements(HTML_QuickForm2_Container $fieldSet, $plan)
    {
        ...
        $fieldSet->addPeriod($prefix . 'first_period' . $suffix)
        ...
        $group->addText($prefix . 'rebill_times' . $suffix, ['size' => 3, 'maxlength' => 6])->setId('t_rebill_times');
        ...
        $gr->addStatic()
            ->setContent(' <span class="billing-plan-currency"></span>');
        $fieldSet->addPeriod($prefix . 'second_period' . $suffix)
                ->setLabel(___('Second Period'));

        $fieldSet->addText($prefix . 'terms' . $suffix, ['size' => 40, 'class' => 'translate'])
        ...
        if (Am_Di::getInstance()->config->get('product_paysystem')) {
            $fieldSet->addMagicSelect($prefix . 'paysys_id' . $suffix, ['class' => 'bp-paysys'])
        ...
        $fieldSet->addCategory($prefix . '_categories' . $suffix, null, [
            'base_url' => 'admin-billing-plan-categories',
            'link_title' => ___('Edit Categories'),
            'title' => ___('Billing Plan Categories'),
            'options' => Am_Di::getInstance()->billingPlanCategoryTable->getOptions()
        ])
            ->setLabel(___('Billing Plan Categories'));

        foreach (Am_Di::getInstance()->billingPlanTable->customFields()->getAll() as $f) {
    ...
    function insertAdditionalFields()
    {
        ...
        foreach ($fields as $f) {
            if (!in_array($f->name, $exclude)) {
                $f->addToQf2($this->getAdditionalFieldSet());
            }
        }
    }
...
class Am_Grid_Filter_Product extends Am_Grid_Filter_Abstract
{
    ...
    public function renderInputs()
    {
        ...
        $options = $this->grid->getDi()->productCategoryTable->getAdminSelectOptions(
                [ProductCategoryTable::COUNT => true]);
        $optionsHtml = Am_Html::renderOptions(
                $options,
                $this->getParam('cat_id'));

        $category = empty($options) ?
            '' :
            <<<CUT
<div style='display:table-cell; padding-right:0.4em; padding-bottom:0.4em; width:200px; box-sizing:border-box;'>
<select name="_product_cat_id[]" style="width:200px" class="magicselect am-combobox" multiple="multiple" data-offer="{$offer}">
{$optionsHtml}
</select>
</div>
CUT;
...
class Am_Grid_Filter_ProductUpgrade extends Am_Grid_Filter_Abstract
{
    protected $varList = [
        'f', 't', 'h_d'
    ];
    ...
    protected function applyFilter()
    {
        ...
        if ($f = $this->getParam('f')) {
            $query->addWhere('from_billing_plan_id IN (?a)', $f);
        }

        if ($t = $this->getParam('t')) {
            $query->addWhere('to_billing_plan_id IN (?a)', $t);
        }

        if ($this->getParam('h_d')) {
            $query->addWhere("is_disabled=0");
        }
    }
    ...
    public function renderInputs()
    {
        ...
        $offer_f = '-- ' . ___('From');
        $offer_t = '-- ' . ___('To');
        $options = $this->grid->getDi()->billingPlanTable->getOptions();
        $options_f = Am_Html::renderOptions(
            $options,
            $this->getParam('f'));
        $options_t = Am_Html::renderOptions(
            $options,
            $this->getParam('t'));
        $disabled = $this->renderShowDisabled();
        ...
        return <<<CUT
<div style='display:table-cell; padding-right:0.4em; padding-bottom:0.4em; width:200px; box-sizing:border-box;'>
<select name="{$prefix}_f[]" style="width:200px" class="magicselect am-combobox" multiple="multiple" data-offer="{$offer_f}">
$options_f
</select>
</div>
<div style='display:table-cell; padding-bottom:0.4em; width:160px; box-sizing:border-box;'>
<select name="{$prefix}_t[]" style="width:200px" class="magicselect am-combobox" multiple="multiple" data-offer="{$offer_t}">
$options_t
</select>
</div>
$disabled
CUT;
    ...
    {
        return sprintf('<label>
                <input type="hidden" name="%s_h_d" value="0" />
                <input type="checkbox" name="%s_h_d" value="1" %s /> %s</label>',
            $this->grid->getId(), $this->grid->getId(),
            ($this->getParam('h_d') == 1 ? 'checked' : ''),
            Am_Html::escape(___('hide disabled upgrades'))
        );
    }
}

class Am_Grid_Action_Group_ProductAssignCategory extends Am_Grid_Action_Group_Form
{
    ...
    public function createForm(): Am_Form
    {
        ...
        $form->addMagicSelect("{$prefix}categories", ['class' => 'am-combobox'])
        ...
        $form->addSaveButton(___("Assign Category"));

        return $form;
    }
}

class Am_Grid_Action_Group_ProductRemoveCategory extends Am_Grid_Action_Group_Form
{
    public function __construct($id = null, $title = null)
    {
    ...
    public function createForm(): Am_Form
    {
        ...
        $form->addSaveButton(___("Remove Category"));
...
class Am_Grid_Action_Group_SetProductRequirements extends Am_Grid_Action_Group_Form
{
    public function handleRecord($id, $record)
    {
    ...
    public function createForm(): Am_Form
    {
        ...
        $form->addMagicSelect($prefix . 'require_other', ['class' => 'am-combobox-fixed'])
            ->setLabel(___("To order this product user must have an\n" .
                "when user orders this subscription, it will be checked\n" .
                "that user has at least one of the following subscriptions"
            ))
        ...
        $form->addMagicSelect($prefix . 'prevent_if_other', ['class' => 'am-combobox-fixed'])
            ->setLabel(___("Disallow ordering of this product if user has\n" .
                "when user orders this subscription, it will be checked\n" .
                "that he doesn't have any of the following subscriptions"
            ))
        ...
        $form->addSaveButton(___('Set Requirements'));

        return $form;
    }
}

class Am_Grid_Action_Group_AmendProductRequirements extends Am_Grid_Action_Group_Form
{
    public function handleRecord($id, $record)
    {
        //require_other
    ...
    public function createForm(): Am_Form
    {
        ...
        $form->addSaveButton(___('Amend Requirements'));
...
class Am_Grid_Action_Group_Archive extends Am_Grid_Action_Group_Abstract
{
    ...
    public function __construct($id = null, $title = null)
    {
        parent::__construct("product-archive", ___("Archive"));
    }
...
class Am_Grid_Action_Sort_ProductUpgrade extends Am_Grid_Action_Sort_Abstract
{
    protected function setSortBetween($item, $after, $before)
    {
        $this->_simpleSort($this->grid->getDi()->productUpgradeTable, $item, $after, $before);
    }
}
...
class AdminProductsController extends Am_Mvc_Controller_Grid
{
    ...
    function valuesToForm(& $ret, Product $record)
    {
        ...
        if ($record->isLoaded()) {
            ...
            foreach ($record->getBillingPlans() as $plan) {
                $arr = $plan->toArray();
                $arr['_categories'] = $plan->getCategories();
    ...
    public function updatePlansFromRequest(Product $record, $values, $existing = [])
    {
        ...
        foreach ($plans as $k => & $arr) {
            if (Am_Di::getInstance()->config->get('product_paysystem')) {
                $arr['paysys_id'] = implode(',', $arr['paysys_id'] ?? []);
            }
        ...
        foreach ($existing as $plan)
            if (empty($plans[$plan->pk()])) {
                ...
                $plan->setCategories(empty($plans[$plan->pk()]['_categories']) ? [] : $plans[$plan->pk()]['_categories']);
                $plan->update();
                unset($plans[$plan->pk()]);
            }
        $plan_map = [];
        foreach ($plans as $id => $a) {
            ...
            $plan->insert();
            $plan->setCategories(empty($a['_categories']) ? [] : $a['_categories']);
            $plan_map[$id] = $plan->pk();
        }
    ...
    public function upgradesAction()
    {
        ...
        $grid->actionAdd(new Am_Grid_Action_Sort_ProductUpgrade);
        $grid->actionAdd(new Am_Grid_Action_Group_Delete);
        $grid->setFilter(new Am_Grid_Filter_ProductUpgrade);
        $grid->runWithLayout('admin/layout.phtml');
    }
    ...
    public function createUpgradesForm(Am_Grid_Editable $grid)
    {
        ...
        $form->addAdvcheckbox('only_recurring')->setLabel(___("Only Active Recurring\napply only to active recurring subscriptions"));
...
CUT
        ...
        $form->addText('terms', ['class' => 'am-el-wide translate'])
            ->setLabel(___("Terms Text\nautomatically calculated if empty"));
        $form->addText('comment', ['class' => 'am-el-wide translate'])

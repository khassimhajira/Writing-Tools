...
class Am_Grid_Action_Group_ContentAssignCategory extends Am_Grid_Action_Group_Form
{
...
class Am_Grid_Action_Group_ContentRemoveCategory extends Am_Grid_Action_Group_Form
{
    public function __construct($id = null, $title = null)
    {
...
class Am_Grid_Action_Group_ContentSetAccessPermission extends Am_Grid_Action_Group_Form
{
    public function __construct()
    {
    ...
    public function createForm(): Am_Form
    {
        $id = $this->grid->getId();
        $form = new Am_Form_Admin;
        $form->addElement(new Am_Form_Element_ResourceAccess)->setName($id . '_access')->setLabel(___('Access Permissions'));
        $form->addSaveButton(___('Set Access Permissions'));

        return $form;
    }
}

class Am_Grid_Action_Group_ContentAmendAccessPermission extends Am_Grid_Action_Group_Form
{
    public function __construct()
    {
    ...
    public function createForm(): Am_Form
    {
        ...
        $form->addMagicSelect($id . '_remove')
            ->setLabel(___('Remove'))
            ->loadOptions(Am_Di::getInstance()->productTable->getProductOptions())
            ->setJsOptions('{allowSelectAll: true}');
        $form->addSaveButton(___('Amend Access Permissions'));
...
abstract class Am_Grid_Filter_Content extends Am_Grid_Filter_Abstract
{
    ...
    function renderInputs()
    {
        ...
        $filter .= $this->renderInputSelect('filter_a',
            ...
            ['style'=>'max-width:300px', 'class' => 'am-combobox']);
...
class Am_Grid_Editable_Pages extends Am_Grid_Editable_Content
{
    ...
    public function initActions()
    {
        ...
        $this->actionAdd(new Am_Grid_Action_Group_ContentAmendAccessPermission);
        $this->actionAdd(new Am_Grid_Action_Group_Delete);
        $this->actionAdd(new Am_Grid_Action_CopyPage);
    }
...
class Am_Grid_Editable_Folders extends Am_Grid_Editable_Content
{
    ...
    public function initActions()
    {
        ...
        $this->actionAdd(new Am_Grid_Action_Group_ContentShowOnDashboard);
        $this->actionAdd(new Am_Grid_Action_Group_Delete);
        $this->actionAdd(new Am_Grid_Action_Group_Callback('re-protect', ___('Re-Protect'), function($id, $record) {
    ...
    public function getHtaccessRewriteFile(Folder $folder)
    {
        ...
        $remote_addr = (defined('AM_NEW_REWRITE_VALIDATE_IP') && AM_NEW_REWRITE_VALIDATE_IP) ? '%{REMOTE_ADDR}-' : '';

        $new_rewrite_url = $this->getDi()->surl('protect/new-rewrite', false);
...
RewriteCond $rd/new-rewrite/$remote_addr%1-{$folder->folder_id} -f
RewriteRule ^(.*)\$ - [S=3]
...
class Am_Grid_Action_Group_ContentChangeOrder extends Am_Grid_Action_Group_Abstract
{
    ...
    public function renderConfirmationForm($btn = null, $addHtml = null)
    {
        ...
        $name = $_->getName();
        $key = $_->getKeyField();
        $resource_type = $_->getAccessType();
        $field = $name == '?_email_template' ? 'subject' : 'title';
        $options = $this->grid->getDi()->db->selectCol(<<<CUT
            SELECT ?#, ?# AS ARRAY_KEY
                FROM $name
                LEFT JOIN ?_resource_access_sort ON resource_id=?# AND resource_type = ?
                {WHERE name IN (?a)}
                ORDER BY sort_order;
CUT
            , $field, $key, $key, $resource_type, $name == '?_email_template' ? ['autoresponder', 'expire', 'productwelcome', 'payment'] : DBSIMPLE_SKIP);

        $select = sprintf('%s <select name="%s__resource_id" class="am-combobox">
            %s
            </select><br /><br />'.PHP_EOL,
...
class Am_Grid_Action_CopyEmail extends Am_Grid_Action_Abstract
{
    ...
    public function run()
    {
        ...
        $controller = new AdminContentController(new Am_Mvc_Request, new Am_Mvc_Response, ['di' => Am_Di::getInstance()]);
...
class Am_Grid_Action_CopyPage extends Am_Grid_Action_Abstract
{
    protected $id = 'copy';
    protected $privilege = 'insert';
    ...
    public function run()
    {
        ...
        $vars = $record->toRow();
        unset($vars['page_id']);
        $vars['title'] = ___('Copy of') . ' ' . $vars['title'];

...
class AdminEmailTemplatesController extends Am_Mvc_Controller
{
    ...
    protected function getCopiedTpl($copy_from)
    {
        ...
        if (!$sourceTpl) {
            throw new Am_Exception_InputError(___('Trying to copy from not existing template: %s', $copy_from));
        }
    ...
    protected function createActionsForm(EmailTemplate $tpl)
    {
        ...
        //we do not show action's form if there is not any available action
        if (!count($langOptions))
        {
            $form = null;
        }

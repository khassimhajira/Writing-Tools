...
class AdminSavedFormController extends Am_Mvc_Controller_Grid
{
    ...
    function init()
    {
        if (!class_exists('Am_Form_Brick', false)) {
            ...
            $this->getDi()->hook->call(Am_Event::LOAD_BRICKS);
        }
...
CUT;
        ...
        $grid->setFilter(new Am_Grid_Filter_SavedForm);
...
class Am_Grid_Filter_SavedForm extends Am_Grid_Filter_Abstract
{
    protected $varList = [
        'q', 't',
    ];
    ...
    protected function applyFilter()
    {
        ...
        if ($q = $this->getParam('q')) {
            $query->add(new Am_Query_Condition_Field('title', 'LIKE', '%' . $q . '%'))
                ->_or(new Am_Query_Condition_Field('comment', 'LIKE', '%' . $q . '%'))
                ->_or(new Am_Query_Condition_Field('code', 'LIKE', '%' . $q . '%'));
        }
        if ($t = $this->getParam('t')) {
            $query->getDataSourceQuery()->addWhere('type=?', $t);
        }
    }

    public function renderInputs()
    {
        return $this->renderInputSelect('t', ['' => '-- Type'] + $this->grid->getDi()->savedFormTable->getTypeOptions())
            . ' '
            . $this->renderInputText(['name' => 'q', 'placeholder' => ___('Title/Comment/Code')]);
    }
}
...
class Am_Grid_EU_Vat_Report_Details extends Am_Grid_Editable
{
    ...
    function init()
    {
        ...
        $this->addField('address', ___('Address'))->setGetFunction(function($record, $grid, $field) {
                return implode(", ", [$record->street, $record->city, $record->zip, $record->state, $record->country]);
            });
        $this->addField('inv_ip', ___('IP (Country)'))->setGetFunction(function($record, $grid, $field) {
                return $record->inv_ip . " (" . ($record->inv_ip_country ?: ___('undefined')) . ")";
            });
        $this->addField('reg_ip', ___('Registration IP (Country)'))->setGetFunction(function($record, $grid, $field) {
                return $record->reg_ip . " (" . ($record->reg_ip_country ?: ___('undefined')) . ")";
            });
        ...
        $this->totals[] = $this->addField('total_amount', ___('Amount excl. Tax'), true, 'right');
        $this->totals[] = $this->addField('tax_amount', ___('VAT'), true, 'right');
        $this->addField('tax_rate', ___('VAT Rate'), true, 'right')->setGetFunction(function($record, $grid, $field) {
    ...
    {
        return ($record->{$field} < 0 ? "-" : "&nbsp;").Am_Currency::render(abs(floatval($record->{$field})));
    }
...
class Am_Grid_EU_Vat_Action_Back extends Am_Grid_Action_Abstract
{
    ...
    protected $url;
    protected $attributes = ['target' => '_top'];
    ...
    function init()
    {
        ...
        $this->addField('country', ___('Country'))->setGetFunction(function($record, $grid, $field) {
                ...
                if (!isset($countries[$record->country]))
                    $countries[$record->country] = $this->getDi()->countryTable->getTitleByCode($record->country);

                return $record->country . " (" . $countries[$record->country] . ")";
            });
        $this->totals[] = $this->addField('sales_amount', ___('Consumer Sales'), true, 'right')->setGetFunction([$this, 'getAmount']);
        $this->totals[] = $this->addField('sales_without_tax_amount', ___('Amount excl. VAT'), true, 'right')->setGetFunction([$this, 'getAmount']);
        $this->addField('tax_rate', ___('VAT Rate'), true, 'right')->setGetFunction(function($record, $grid, $field){
            return $record->{$field} ? $record->{$field}."%" : "-";
        });
        $this->totals[] = $this->addField('tax_amount', ___('VAT Amount'), true, 'right')->setGetFunction([$this, 'getAmount']);
...
class Am_Grid_Action_EU_VAT_RowDetails extends Am_Grid_Action_Abstract
{
    ...
    function getUrl($record = null, $id = null)
    {
        ...
        $params = [
            ...
            'return_url'=>  $this->grid->getDi()->request->assembleUrl(false,true),
        ];
        if (!is_null($record)) {
            $params['_eu_vat_details_filter']['country'][] = $record->country;
            $params['_eu_vat_details_filter']['tax_rate'] = $record->tax_rate;
        }
        return $this->grid->getDi()->url("admin-vat-report/details", $params, false);
    }
...
class Am_Grid_Action_EU_VAT_Details extends Am_Grid_Action_EU_VAT_RowDetails
{
    ...
    {
        $out .= sprintf(
            '<div style="float:right">&nbsp;&nbsp;<a target="_top" href="%s">'.___('Details').'</a></div>',
                $this->getUrl()
        );
    }
}
...
class Am_Grid_Filter_EU_VAT_Report extends Am_Grid_Filter_EU_VAT
{
    ...
    function renderInputs()
    {
        ...
        $countryOptions = [];
        $countryOptions = $countryOptions + $this->grid->getDi()->countryTable->getOptions();
        $countryOptions = Am_Html::renderOptions($countryOptions, @$filter['country']);

        $paysysOptions = [];
        $paysysOptions = $paysysOptions + $this->grid->getDi()->paysystemList->getOptions();
        ...
        $pOptions = Am_Html::renderOptions($pOptions, @$filter['period']);
        ...
        return <<<CUT
    <div>
        <div style='display:table-cell; padding-right:0.4em;'>
            $pSelect
            <select name='{$prefix}_filter[period]' id='filter-period'>
                {$pOptions}
            </select>
        </div>
        <div style='display:table-cell; padding-right:0.4em;' id='filter-exact'>
        ...
        <div style='display:table-cell; padding-right:0.4em; width:160px; box-sizing:border-box;'>
            <select name="{$prefix}_filter[country][]" style="width:160px" class="magicselect" multiple="multiple" data-offer='{$offer_country}'>
                $countryOptions
            </select>&nbsp;&nbsp;
        </div>
        <div style='display:table-cell;  width:160px; box-sizing:border-box;'>
    ...
    <script>
        jQuery(document).ready(function(){
            ...
            jQuery('#filter-period').change(function(){
                jQuery("#filter-exact").toggle(jQuery(this).val() == 'exact')
            }).change();
        });
    </script>
CUT;
    ...
    public function renderTotals()
    {
        ...
        foreach ($this->fields as $field)
        {
            ...
            $totals[] = sprintf(
                ...
                ___('Total'),
                $field->getFieldTitle(),
                Am_Currency::render($this->grid->getDi()->db->selectCell("SELECT SUM(total_qry.$name) FROM ($sql) AS total_qry"))
            );
        }

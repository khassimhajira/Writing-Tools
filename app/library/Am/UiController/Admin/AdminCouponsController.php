    ...
    function init()
    {
        ...
        $this->addInteger('use_count')
            ->setLabel(___("Coupons Usage Count\n" .
                'how many times single coupon can be used, keep empty for unlimited'));
        ...
        if (!$this->record->isLoaded()) {
            ...
            $source->addAdvRadio('_source')
                ...
                ->loadOptions([
                    ...
                    AdminCouponsController::SOURCE_EMPTY => ___('Empty Batch'),
                ]);
            ...
            $prefix = $source->addText('_prefix', ['rel' => 'source-generate', 'size' => 5])
                ->setLabel(___("Code Prefix"));
            $prefix->addRule('callback2', null, [$this, 'checkPrefix']);
...
CUT
        ...
        $bp_categories = [];
        foreach (Am_Di::getInstance()->billingPlanCategoryTable->getOptions() as $id => $title) {
            $bp_categories["BPCATEGORY-{$id}"] = $title;
        }
        ...
        $fs->addMagicSelect('product_ids', ['class' => 'am-combobox-fixed'])
            ->loadOptions([
                ...
                ___('Billing Plans Categories') => $bp_categories,
            ])
...
CUT;
    ...
    function checkCouponCode($v)
    {
        ...
        if ($vars['_source'] == AdminCouponsController::SOURCE_INPUT) {
            ...
            if (!Am_Di::getInstance()->couponTable->validateCode($v)) return ___('Invalid coupon code');
        }
    }
    ...
    function checkPrefix($v)
    {
        $vars = $this->getValue();
        if ($vars['_prefix']) {
            if (Am_Di::getInstance()->config->get('coupon_no_special_chars') && !preg_match("/^[0-9a-zA-Z]+$/", $vars['_prefix'])) {
                return ___('Invalid prefix');
            }
        }
    }
}
...
class Am_Grid_Filter_Coupon extends Am_Grid_Filter_Abstract
{
    ...
    {
        return $this->renderInputText(['placeholder' => ___('Coupon Code')])
            . ' '
            . $this->renderInputSelect('pid', ['' => ___('-- Product')] + $this->grid->getDi()->productTable->getOptions(), ['class' => 'am-combobox-fixed']);
    }
}

class Am_Grid_Action_Group_CouponBatchEdit extends Am_Grid_Action_Group_Form
{
    public function __construct()
    {
        parent::__construct('g-edit', ___('Edit'));
        $this->setTarget('_top');
    }

    public function handleRecord($id, $record)
    {
        $record->discount = $this->_vars['discount'];
        $record->discount_type = $this->_vars['discount_type'];
        $record->save();
    }
    ...
    public function createForm(): Am_Form
    {
        ...
        $discountGroup = $this->form->addGroup()
            ->setLabel(___('Discount'));
        $discountGroup->setSeparator(' ');
        $discountGroup->addText("{$id}_discount", ['size' => 5]);
        $discountGroup->addSelect("{$id}_discount_type")
            ->loadOptions([
                Coupon::DISCOUNT_PERCENT => '%',
                Coupon::DISCOUNT_NUMBER => Am_Currency::getDefault()
            ]);

        $form->addSaveButton(___('Save'));
...
class AdminCouponsController extends Am_Mvc_Controller_Grid
{
    ...
    public function createGrid()
    {
        ...
        $grid->actionAdd(new Am_Grid_Action_Group_CouponBatchEdit);
...
CUT
    ...
    {
        return sprintf("<td align='center' id='batch-{$record->pk()}-usage'>%s</td>",
            $record->use_count ?
                sprintf("%d<span style='color:#aaa'>/%d</span>", $record->used_count, $record->use_count * $record->coupons_count) :
                $record->used_count
        );
    }
    ...
    function renderProducts($record)
    {
        ...
        $bp_ids = $record->getOnlyApplicableBillingPlanIds();
        $bpcategory_ids = $record->getOnlyApplicableBillingPlanCategoryIds();

        if (!$product_ids && !$category_ids && !$bp_ids && !$bpcategory_ids) {
            return sprintf('<td><strong>%s</strong></td>', ___('All'));
        }

        return $this->_renderProducts($product_ids, $category_ids, $bp_ids, $bpcategory_ids);
    }
    ...
    function renderNotProducts($record)
    {
        ...
        $bp_ids = $record->getOnlyNotApplicableBillingPlanIds();
        $bpcategory_ids = $record->getOnlyNotApplicableBillingPlanCategoryIds();
    ...
    function _renderProducts($product_ids, $category_ids, $bp_ids, $bpcategory_ids)
    {
        $res = [];
        ...
        if ($bpcategory_ids) {
            $options = $this->getDi()->billingPlanCategoryTable->getOptions();
            $titles = [];
            foreach ($bpcategory_ids as $id) {
                $titles[] = $options[$id];
            }

            $titles = implode(', ', $titles);
            $res[] = sprintf("<strong>%s:</strong> %s", ___('Billing Plan Categories'), $titles);
        }
    ...
    function beforeSave(& $values)
    {
        ...
        $values['expire_date'] = $values['expire_date'] ?: null;
    }
    ...
    public function afterInsert(array & $values, CouponBatch $record)
    {
        switch ($values['_source']) {
            ...
            case self::SOURCE_FILE :
                ...
                while ($row = fgetcsv($f)) {
                    if(!$this->getDi()->couponTable->validateCode($row[0])) {
                        continue;
                    }
    ...
    public function detailAction()
    {
        ...
        $liveCouponEdit = new Am_Grid_Action_LiveEdit('code');
        $liveCouponEdit->setUpdateCallback(function($ds, $record, $fieldname, $v)
            {
                if(!$this->getDi()->couponTable->validateCode($v)){
                    throw new Am_Exception_InputError(___('Invalid coupon code'));
                }
                $ds->updateRecord($record, [$fieldname => $v]);
            }
        );
        $liveCouponEdit->setCallback(<<<CUT
l = function(val){
    ...
    source: amUrl("/admin-users/autocomplete")
        ...
        $grid->addCallback(Am_Grid_ReadOnly::CB_RENDER_TABLE, function(& $out, $grid) {
            ...
            $content = $cnt == 1 ?
                ...
                $cnt;
            $usage = json_encode($batch->use_count ? sprintf("%d<span style='color:#aaa'>/%d</span>", $used_cnt, $batch->use_count * $cnt) : $used_cnt);
            $out .= <<<CUT
...
CUT;
    ...
    public function createCouponForm()
    {
        ...
        $c->addRule('callback', ___('Invalid coupon code'), function($c) use ($di) {
            return $di->couponTable->validateCode($c);
        });
    ...
    public function getUsedCount($obj)
    {
        ...
        $out = '';
        $url = str_replace('%', '%%', $this->getDi()->url('admin-payments?_payment_filter[text]=' . urlencode($obj->code) . '&_payment_filter[type]=coupon&_payment_filter[datf]=dattm&_payment_filter[dat1]&_payment_filter[dat2]'));
        ...
        foreach ($invoices as $invoice) {
            ...
            foreach ($payments as $payment) {
                ...
                $ret[] = sprintf($tpl,
                    ...
                    $this->getDi()->url("admin-user-payments/index/user_id/{$invoice->user_id}#invoice-{$invoice->pk()}", false),
                    $invoice->pk(),
                    $this->getDi()->view->escape($invoice->public_id),

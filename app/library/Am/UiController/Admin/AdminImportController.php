...
class AdminImportController extends Am_Mvc_Controller
{
    ...
    protected Am_Upload $upload;
    protected Am_Session_Ns $session;
    protected ?Am_Import_DataSource $dataSource;
    protected Am_Import_Log $log;
    ...
    public function _runAction($action)
    {
        ...
        if ($action == 'backAction') {
            switch ($this->session->step) {
                case 2:
                    if (!empty($this->session->uploadSerialized)) {
    ...
    public function indexAction()
    {
        ...
        $grid = $this->createDemoHistoryGrid();
        $grid->isAjax($this->isAjax() && $this->isGridRequest('_h'));
    ...
    public function assignAction()
    {
        ...
        if ($submitted) {
            ...
            $form = $this->getForm(self::FORM_ASSIGN, $recreate = true, $force_submitted = true);
        }
        ...
        if ($submitted && !($error = $this->validateAssign())) {
            $this->defineAction();
        } else {
    ...
    protected function validateAssign()
    {
        ...
        if (
            isset($this->session->importOptions['add_subscription'])
        ...
        if (
            isset($this->session->importOptions['add_ecrypted_pass'])
        ...
        //let's check if one field was assigned to more than one column
    ...
    function renderGridTitle($record)
    {
        return $record->completed ?
            sprintf('<td>%s</td>', ___('You have imported %d customers', $record->user_count)) :
            sprintf('<td>%s</td>', ___('Import of data was terminated while processing. Anyway some data was imported.'));
    }
    ...
    protected function sendDelRedirect()
    {
        ...
        $this->redirectHtml($this->getUrl('admin-import', 'do-delete'), ___('Clean up data. Please wait...'), ___('Clean up...'), false, $processed, $total);
    }
    ...
    protected function sendRedirect()
    {
        ...
        $processed = $this->log->getStat(Am_Import_Log::TYPE_PROCCESSED);
        $total = $this->dataSource->getEstimateTotalLines($processed);
        $this->redirectHtml($this->getUrl('admin-import', 'import'), ___('Import data. Please wait...'), ___('Import...'), false, $processed, $total);
    }

    protected function getID()
    {
        if (!$this->session->ID) {
            $this->session->ID = sprintf('I-%s', strtoupper($this->getDi()->security->randomString(6)));
        }
    ...
    protected function addSub(Am_Record $user, $lineParsed)
    {
        ...
        if (empty($this->session->importOptions['skip_invoice'])) {
    ...
    {
        return $this->importFields[$type][$fieldName] ?? null;
    }
    ...
    protected function createDefineForm($force_submitted = false)
    {
        ...
        if ($force_submitted || $form->isSubmitted()) {
    ...
    protected function createConfirmForm($force_submitted = false)
    {
    ...
    protected function createUploadForm($force_submitted = false)
    {

...
class Am_Grid_Action_Group_ForcePasswordChange extends Am_Grid_Action_Group_Abstract
{
    protected $privilege = 'edit';
    protected $title = 'Force Password Change';

    public function handleRecord($id, $record)
    {
        $record->data()->set('force_change_password', 1)->update();
    }
}

class Am_Grid_Action_Group_UserAssignGroup extends Am_Grid_Action_Group_Form
{
    ...
    public function handleRecord($id, $record)
    {
        ...
        $user->setGroups(array_merge($groups, $this->_vars['groups']));
        $user->save();
    }
    ...
    public function createForm(): Am_Form
    {
        ...
        $form->addMagicSelect("{$prefix}groups", ['class' => 'am-combobox'])
            ->setLabel(___("Assign Group"))
            ->loadOptions(Am_Di::getInstance()->userGroupTable->getSelectOptions())
            ->setJsOptions('{allowSelectAll: true}');

        $form->addSaveButton(___("Assign Group"));

        return $form;
    }
}

class Am_Grid_Action_Group_UserRemoveGroup extends Am_Grid_Action_Group_Form
{
    public function __construct($id = null, $title = null)
    {
    ...
    public function createForm(): Am_Form
    {
        ...
        $form->addSaveButton(___("Remove Group"));
...
class Am_Grid_Action_Group_ResendConfirmationLink extends Am_Grid_Action_Group_Abstract
{
    ...
    public function getIds()
    {
        ...
        return array_filter(array_map(fn($_) => preg_replace('/[^a-zA-Z0-9]/', '', $_), $ids));
    }
}
...
class Am_Grid_Action_Group_PasswordConfirmedDelete extends Am_Grid_Action_Group_Delete
{
    ...
    public function renderConfirmationForm($btn = null, $addHtml = null)
    {
        if (!$this->getSession()->login_ok) {
            $addHtml = ___('Enter admin password for confirmation') .
                ":<br /><input type='password' name='_admin_pass' size=20 spellcheck='false' /><br /><br />" . $addHtml;
        } else {
            $addHtml = null;
        }
    ...
    {
        try {
            parent::handleRecord($id, $record);
        } catch(Am_Exception_User_Has_Recurring $e) {
            $this->grid->getDi()->errorLogTable->logException($e);
        }
    }
...
class Am_Form_Admin_User extends Am_Form_Admin
{
    ...
    function checkUniqLogin($login)
    {
        ...
        // We need to check login only when user is not exists, or when he changes his username.
    ...
    function checkUniqEmail(array $group)
    {
        ...
        // We need to check email only when user is not exists, or when he changes his email.
        $user_id = $this->record ? $this->record->pk() : null;
...
CUT
        ...
        if (!$this->record->isLoaded()) {
            ...
            $g->addPassword('_pass', ['autocomplete' => 'new-password', 'spellcheck' => 'false'])
                ->setLabel(___('Password'))
                ->addRule('required');
        } else {
            $g = $fieldSet->addGroup('', ['id' => 'pass-group'])
...
class Am_Grid_Action_Group_MassSubscribe extends Am_Grid_Action_Group_Form
{
    ...
    {
        $id = $this->grid->getId();
        $form = new Am_Form_Admin('mass-subscribe');
        $form->addSelect($id . '_product_id', ['class' => 'am-combobox-fixed'])
            ->setLabel(___('Product'))
            ->loadOptions($this->grid->getDi()->productTable->getOptions(false))
            ->addRule('required');

        $actionElName = $id.'_action';
        $form->addAdvRadio($actionElName)
            ->setLabel('Action')
            ->loadOptions([
        ...
        $fs = $form->addFieldset(null, ['id' => 'action-access']);
...
CUT
        ...
        $fs = $form->addFieldset(null, ['id' => 'action-payment']);
        ...
        $form->addText($id.'_comment', ['class'=>'am-el-wide'])
            ->setLabel(___("Comment\nfor admin reference"));
        ...
        $form->addSaveButton(___('Mass Subscribe'));
...
class Am_Grid_Action_Group_AddUserNote extends Am_Grid_Action_Group_Form
{
    public function __construct()
    {
    ...
    {
        $id = $this->grid->getId();
        $form = new Am_Form_Admin($this->getId());

        $form->addTextarea("{$id}_content", ['rows' => 10, 'class' => "am-no-label am-el-wide"])
            ->setLabel(___('Message'));

        $form->addUpload("{$id}_attachments", ['multiple' => 1], ['prefix' => 'user_note'])
...
CUT;
    ...
    protected function merge(User $source, User $target)
    {
        ...
        $_log = ___('Merged users, source: %s <%s> (%s) [#%d], target: %s <%s> (%s) [#%d]',
            $source->getName(),
            $source->email,
            $source->login,
            $source->pk(),
...
class Am_Grid_Filter_User extends Am_Grid_Filter_Abstract
{
    ...
    public function  renderFilter()
    {
        ...
        preg_match('#(<div class="am-filter-wrap">)(.*)$#is', $filter, $matches);
...
class Am_Grid_Filter_NotConfirmed extends Am_Grid_Filter_Abstract
{
    protected function applyFilter()
    {
        ...
        [$fieldname, $desc] = $this->grid->getDataSource()->_friendGetOrder();
        if ($fieldname) {
...
CUT
    ...
    public function notConfirmedAction()
    {
        ...
        $grid->addField('expires', ___('Expires'))
            ->setRenderFunction(function($r, $fn, $g, $fo) {
                ...
                $left = $_ > 60 ? ___('in %s hour(s)', floor($_/60)) : ___('in %s minute(s)', floor($_));

                return $g->renderTd(sprintf('%s <span style="margin-left:1em; opacity:.5;">%s</span>', amDatetime($r->$fn), $left), false);
            });
    ...
    public function createGrid()
    {
        ...
        $grid->actionAdd(new Am_Grid_Action_Group_ForcePasswordChange);
    ...
    {
        if ($record->isLocked() || !$record->isApproved()) {
    ...
    protected function createActionCustomize()
    {
        ...
        $mobileField = (new Am_Grid_Field('mobile', ___('Mobile Number'), false))->setGetFunction(fn(User $user) => $user->getMobile());
    ...
    protected function createActionExport()
    {
        ...
        $action->addField(new Am_Grid_Field('user_id', ___('User Id')))
                ...
                ->addField(new Am_Grid_Field('name_f', ___('First Name')))
                ->addField(new Am_Grid_Field('name_l', ___('Last Name')))
                ->addField(new Am_Grid_Field('_name', ___('Full Name (First and Last)')))
                ->addField(new Am_Grid_Field('street', ___('Street')))
                ->addField(new Am_Grid_Field('street2', ___('Street (Second Line)')))
                ->addField(new Am_Grid_Field('city', ___('City')))
                ->addField(new Am_Grid_Field('state', ___('State')))
    ...
    function renderLogin($record, $fieldName, $grid, $field)
    {
        $icons = "";
        if ($record->isLocked())
            $icons .= sprintf('<i class="fa-solid fa-lock" title="%s"></i>', ___('User is locked'));
        if (!$record->isApproved())
            $icons .= sprintf('<i class="fa-solid fa-circle-exclamation" title="%s"></i>', ___('User is not approved'));
        if ($icons) $icons = '<div style="color: #555960; font-size: .8em; display: flex; gap: .4em; margin-left: auto; align-items: center; padding-left: .4em">' . $icons . '</div>';

        return $this->renderTd(
            sprintf(
                '<div style="display: flex"><a class="link" target="_top" href="%s">%s</a>%s</div>',
                $grid->escape($grid->getActionUrl('edit', $record->user_id)),
                $grid->escape($record->login),
                $icons,
            ), false);
    }
...
CUT
    ...
    function resendSignupEmailAction()
    {
        ...
        $this->getDi()->adminLogTable->log("Resend Signup Email", 'grid_u', $user->pk());

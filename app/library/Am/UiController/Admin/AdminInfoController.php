...
CUT;
    ...
    function indexAction()
    {
        ...
        $r = $this->getDi()->db->selectRow("SHOW STATUS LIKE 'uptime'");
        $mysqlUptime = $this->getDi()->view->formatPeriod($r['Value']);
        $mysqlInnodbBufferPoolSize = $this->getDi()->view->formatBytes((int)$this->getDi()->db->selectCell("SELECT @@innodb_buffer_pool_size"));
...
<script>
jQuery(function($){
    $.get($upgrade_url, function(r){
        $('#am-version-cell').append(r)
    })
})
</script>
...
<tr class="am-grid-row odd">
    <td align="right">innodb_buffer_pool_size</td>
    <td><strong>$mysqlInnodbBufferPoolSize</strong></td>
</tr>
<tr class="am-grid-row odd">
    <td align="right">Uptime</td>
    <td><strong>$mysqlUptime</strong></td>
</tr>
<tr class="am-grid-row">
...
CUT;
            ...
            $content .= sprintf('<style>%s</style><h1>PHP Info</h1><div id="phpinfo" class="am-grid-container"><br />%s</div>',
                $phpStyles, $phpInfo);

...
{
    /** @var Am_Query_Ui */
    protected Am_Query_Ui $searchUi;
    ...
    function renderUserUrl(User $user)
    {
        ...
        return sprintf(
            ...
            Am_Html::escape($url),
            Am_Html::escape($user->login)
        );
    }

    function browseUsersAction()
    {
        $withWrap = (bool) $this->_request->get('_ebu_wrap');
        unset($_GET['_ebu_wrap']);

        $ds = $this->searchUi->getActive()->getQuery();

        $grid = new Am_Grid_ReadOnly('_ebu', ___('Selected for E-Mailing'), $ds,
    ...
    function testAction()
    {
        ...
        if (!Am_Validate::email($this->getParam('email'))) {
            $err_text = ___('Enter valid email address to send test message');
            echo '<span class="am-error">' . Am_Html::escape($err_text) . '</span>';
            return;
        }
...
CUT
    ...
    protected function getReplyToOptions()
    {
        ...
        foreach (Am_Di::getInstance()->adminTable->findBy() as $admin) {
           $op['admin-' . $admin->pk()] = sprintf('%s <%s>', $admin->getName(), $admin->email);
        }
        $op['other'] = ___('Other') . ':';
        return $op;
    }
...
CUT
    ...
    function historyRowsAction()
    {
        ...
        $grid->setFilter(new Am_Grid_Filter_Text(null, ['subject' => 'LIKE'], ['placeholder' => ___('Subject')]));
        $grid->run($this->getResponse());
    }
    ...
    function doSend($r)
    {
        ...
        $tpl->assignStdVars();
        if (
            (strpos($body, '%user.unsubscribe_link%') !== false)
        ...
        try {
            if ($this->getDi()->hook->have(Am_Event::MAIL_SIMPLE_TEMPLATE_BEFORE_SEND)) {
                $user = $this->getDi()->userTable->load($r['user_id']);
                $this->getDi()->hook->call(Am_Event::MAIL_SIMPLE_TEMPLATE_BEFORE_SEND, [
                    'mail' => $m,
                    'recipient' => $user,
                    'recepient' => $user, //backward compatibility
                ]);
            }
            $m->send();
    ...
    function batchSend(&$context, Am_BatchProcessor $batch)
    {
        ...
        $this->searchUi->getActive()->getQuery()->distinct()->setOrder('user_id');

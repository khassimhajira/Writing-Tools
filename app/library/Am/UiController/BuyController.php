...
class BuyController extends Am_Mvc_Controller
{
    function indexAction()
    {
        ...
        $h = $this->_request->getFiltered('h');

        if ($user_id)
        {
            $invoice = $this->createInvoice($btn, $user_id, $this->getParam('coupon'));

            $ps = $this->getPaysystems($btn, $invoice);

            if (
                ($btn->use_coupons && !$this->getParam('coupon'))
                || count($ps) > 1
                || $invoice->getItem(0)->tryLoadProduct()->getOptions()
                || $invoice->getItem(0)->tryLoadProduct()->getBillingPlan()->variable_qty
            ) {

                [$form, $variable_qty, $op]  = $this->createForm($btn, $invoice, $ps);

                if ($form->isSubmitted() && $form->validate()) {
                    $vars = $form->getValue();
                    $this->buildInvoice($invoice, $vars, $variable_qty, $op);
                } else {
    ...
    function previewAction()
    {
        ...
        if ($user_id) {
            ...
            $this->buildInvoice($invoice, $vars, $variable_qty, $op);
            $this->view->invoice = $invoice;
            $this->view->display('_receipt.phtml');
        }
    }
    ...
    protected function getUserId(): ?int
    {
        ...
        if (!($user_id = $this->getDi()->auth->getUserId())) {
            if (
                $this->getSession()->signup_member_id
                && $this->getSession()->signup_member_login
            ) {
                $user = $this->getDi()->userTable->load((int)$this->getSession()->signup_member_id, false);
                if (
                    $user
                    && (
                        (($this->getDi()->time - strtotime($user->added)) < 24*3600)
    ...
    protected function createForm(Button $btn, Invoice $invoice, array $ps): array
    {
        $form = new Am_Form('pay-form');
        $form->addCsrf();

        $variable_qty = $invoice->getItem(0)->tryLoadProduct()->getBillingPlan()->variable_qty;
        ...
        if ($btn->use_coupons) {
            ...
            $coupon->addRule('callback2', null, function($value) use ($invoice) {
                return $this->validateCoupon($value, $invoice);
            })
                ->addRule('remote', null, [
        ...
        $form->addDataSource(new HTML_QuickForm2_DataSource_Array(['qty' => 1]));

        return [$form, $variable_qty, $op];
    }

    protected function createInvoice($btn, $user_id, $coupon): Invoice
    {
        $invoice = $btn->createInvoice();
        $invoice->user_id = $user_id;
        if ($btn->use_coupons && $coupon) {
            $invoice->setCouponCode(trim($coupon));
    ...
    protected function buildInvoice($invoice, $vars, $variable_qty, $op)
    {
        if ($variable_qty) {
            $item = $invoice->getItem(0);
            $invoice->deleteItem($item);
            $invoice->add($item->tryLoadProduct(), $vars['qty']);
        }
    ...
    protected function validateCoupon($value, Invoice $invoice)
    {
        if ($value == "")
            return null;
        $coupon = $this->getDi()->couponTable->findFirstByCode($value);
        $msg = $coupon ? $coupon->validate($invoice->user_id) : ___('No coupons found with such coupon code');
        return $msg === null ? null : $msg;
    }

    protected function getPaysystems($btn, $invoice): array
    {
        $paysys = [];
    ...
    protected function insertProductOptions(HTML_QuickForm2_Container $form, $pid, array $productOptions, BillingPlan $plan)
    {

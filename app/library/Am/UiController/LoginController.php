...
class LoginController extends Am_Mvc_Controller_Auth
{
    ...
    public function init()
    {
        ...
        $this->remember_auto = $this->remember_login && $this->getDi()->config->get($this->configBase . '.remember_auto', false);
    ...
    public function changePassAction()
    {
        ...
        $pass_curr = $form->addPassword('pass_current')
            ->setLabel(___('Your Current Password'));
        $pass_curr->addRule('callback', ___('Wrong password'), [$this, 'checkPassword']);
        ...
        $pass->addRule('neq', ___('New Password should be different than your current one'), $pass_curr);
    ...
    protected function getConfiguredRedirect()
    {
        ...
        $cnt = 0;
        $resources = $this->getDi()->resourceAccessTable->getAllowedResources($this->getDi()->user, ResourceAccess::USER_VISIBLE_PAGES);
        if (!$resources)
            return $default;
        if (!$first) {
    ...
    public function getOkUrl()
    {
        return get_first(
            $this->redirect_url,
            $this->getDi()->hook->filter(
                $this->getConfiguredRedirect(),

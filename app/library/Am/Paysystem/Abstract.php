...
CUT
    ...
    public function isNotAcceptableForInvoice(Invoice $invoice)
    {
        ...
        if ((float)$invoice->second_total && !defined('AM_ALL_PAYSYS_RECURRING') && !$this->isRecurring()) {
    ...
    public function changeRebillDate(Invoice $invoice, DateTime $rebill_date)
    {
        ...
        // Extend access accordingly
        if ($invoice->getAccessExpire() < $invoice->rebill_date)
            $invoice->extendAccessPeriod($invoice->rebill_date);

        $invoice->data()->set('first_rebill_failure', null)->set('last_rebill_failure', null)->update();
    }
    /**

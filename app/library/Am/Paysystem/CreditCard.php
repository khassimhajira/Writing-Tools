...
abstract class Am_Paysystem_CreditCard extends Am_Paysystem_Abstract
{
    ...
    public function prorateInvoice(Invoice $invoice, CcRecord $cc, Am_Paysystem_Result $result, $date)
    {
        ...
        if (!$reattempt) {
            ...
            $this->getDi()->hook->call(Am_Event::INVOICE_AFTER_FAILED, ['invoice' => $invoice]);
            return;
        }
        ...
        $_ = $p->addTo($first_failure);
        $max_rebill_date = sqlDate("$_ -1 day");

        $days_diff = round((strtotime($date) - strtotime($first_failure)) / (24*3600));
        foreach ($reattempt as $day)
             if ($day > $days_diff) break; // we have found next re-bill date to jump

        $rebill_date = date('Y-m-d', strtotime($first_failure." +$day days"));

        if ($day <= $days_diff || $rebill_date > $max_rebill_date){
            // Several re-billing attempts failed already
            // or prorate period more than billing cycle
        ...
        $invoice->updateQuick('rebill_date', $rebill_date);
        ...
        try
        {
            if($et = Am_Mail_Template::load('cc.rebill_failed'))
            {
                ...
                $et->setManual_rebill_link(
                    $this->isManualRebillPossible($invoice) ? sprintf(___("Please use this link to complete your subscription payment manually: %s"),
                        $this->getDi()->surl('login', ['_amember_redirect_url' => base64_encode($this->getManualRebillUrl($invoice))])) : ""
                );
    ...
    {
        //use onPaymentWithAccessAfterInsert instead - this code should be executed on manual-rebill too.
    ...
    function onPaymentWithAccessAfterInsert(Am_Event $e)
    {
        ...
        if ($payment->paysys_id == $this->getId() && !$payment->isFirst()) {
            $this->_onRebillSuccess($e->getInvoice());
        }
    }

    public function _onRebillSuccess(Invoice $invoice)
    {
...
EOT
        ...
        if($this->getRecurringType() == self::REPORTS_CRONREBILL)
        {
            ...
            $el->setLabel(___("Retry On Failure\n".
                ...
                "numbers which is more than billing cycle is ignored"));

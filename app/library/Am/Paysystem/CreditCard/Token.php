...
abstract class Am_Paysystem_CreditCard_Token extends Am_Paysystem_CreditCard implements Am_Paysystem_TokenPayment
{
    ...
    {
        try {
            $invoice = $this->findInvoiceForTokenUpdate($cc->user_id);
            $token = $this->validateToken($invoice, $cc->getToken());
            $invoices = $this->findInvoicesForTokenUpdate($cc->user_id);
    ...
    function createForm($actionName, $invoice = null)
    {
        ...
        return new Am_Form_CreditCard_Token($this, $actionName == 'cc' ? Am_Form_CreditCard_Token::PAYFORM : $actionName);
    }
    ...
    function onInvoiceBeforePaymentSignup(Am_Event $event)
    {
        ...
        if (
            $this->isCCBrickSupported()
            && !empty($vars)
            && !empty($vars[$this->getJsTokenFieldName()])
        ) {
            try {
    ...
    {
        return $this->getDi()->invoiceTable->findFirstBy([
            'user_id' => $user_id,
    ...
    function findInvoicesForTokenUpdate($user_id)
    {
        return $this->getDi()->invoiceTable->findBy([
            'user_id' => $user_id,
            'paysys_id' => $this->getId(),
            'status' => Invoice::RECURRING_ACTIVE
        ]);
    ...
    function onValidateSavedForm(Am_Event_ValidateSavedForm $event)
    {
        ...
        if (
            $event->getForm() instanceof Am_Form_Profile
            && !empty($vars)
            && !empty($vars[$this->getJsTokenFieldName()])
        ) {
            try {
                $invoice = $this->findInvoiceForTokenUpdate($this->getDi()->auth->getUserId());
                $token = $this->convertPaymentTokenToLifetime($this->getJsTokenFromRequest($vars), $invoice);
                $invoices = $this->findInvoicesForTokenUpdate($this->getDi()->auth->getUserId());
                foreach ($invoices as $invoice) {
                    $this->saveToken($invoice, $token);
                }
            } catch (Am_Exception_Paysystem_TransactionInvalid $ex) {

...
class Am_Lite
{
    ...
    protected $useRememberLogin = false;
    ...
    public function setUseRememberLogin($flag)
    {
        $this->useRememberLogin = (bool) $flag;
    }
...
CUT;
    ...
    public function getAffiliate()
    {
        ...
        $res = $this->query("SELECT * FROM ?_user WHERE user_id=? LIMIT 1", $u['aff_id']);
        if ($aff = $res->fetch()) {
    ...
    {
        return $this->hasIdentity() ? $this->_getAccessRecords(self::ANY) : [];
    }

    public function getAccessCache()
    {
        return $this->hasIdentity() ? $this->_getAccessCache($this->getUserField('user_id')) : [];
    }
    ...
    public function getUserGroupsOptions() {
        $groups = [];
        $res = $this->query("SELECT user_group_id, title
            FROM ?_user_group
            ORDER BY sort_order, title");
        foreach ($res as $r)
        {
            $groups[$r['user_group_id']] = $r['title'];
        }
        return $groups;
    }
    ...
    {
        return $u ? sha1($u['user_id'] . $u['login'] . md5($u['pass']) . $u['remember_key'] . $u['auth_key']) : null;
    }
    ...
    function hasValidRememberMe(): ?array
    {
        if (!empty($_COOKIE['amember_ru']) && !empty($_COOKIE['amember_rp'])) {
            ...
            if ($result) {
                $user = $result->fetch();
                if ($user && hash_equals($this->getLoginCookie($user), $pass)) {
                    return $this->getFullUserRecord($user);
                }
            }
        }
    ...
    protected function authenticate()
    {
        ...
        } elseif ($this->useRememberLogin && ($user = $this->hasValidRememberMe())) {
            $this->identity = $user;
        }
    }
    ...
    {
        return $this->hasIdentity() ? $this->identity : null;
    }

    protected function getUserField($name)
    {
        return $this->hasIdentity() ? $this->getIdentity()[$name] : null;
    }

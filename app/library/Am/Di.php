 ...
 * @property \Am\PdoDb $pdoDb lite/quick database interface
 ...
 * @property BillingPlanCategoryTable $billingPlanCategoryTable
 ...
 * @property VideoTable $videoTable
 ...
 * @property Am_RateLimit $rateLimit - callable object to setup API and other rate limits
 ...
 * @property HelpdeskFaqCategoryTable $helpdeskFaqCategoryTable
 * @property HelpdeskCategoryTable $helpdeskCategoryTable
 * // newsletter
 * @property NewsletterListTable $newsletterListTable
 * @property NewsletterUserSubscriptionTable $newsletterUserSubscriptionTable
 * // self-service
 * @property SelfserviceBalanceTable $selfserviceBalanceTable
 * @property SelfserviceHistoryTable $selfserviceHistoryTable
 * @property SelfserviceResourceAccessTable $selfserviceResourceAccessTable
 * @property SelfserviceTypeTable $selfserviceTypeTable
 * @property SelfserviceTypeProductTable $selfserviceTypeProductTable
 * @property SelfserviceTypeResourceTable $selfserviceTypeResourceTable
 * // softsale
 * @property LicenseTable $licenseTable
 * @property LicenseActivationTable $licenseActivationTable
 * @property LicenseLogTable $licenseLogTable
 * @property LicenseSchemeTable $licenseSchemeTable
 * @property SoftsaleFileTable $softsaleFileTable
 * @property SoftsaleFileUploadTable $softsaleFileUploadTable
 * // subusers
 * @property SubusersInviteTable $subusersInviteTable
 * @property SubusersSubscription $subusersSubscription
 *
 ...
 * @property-read HelpdeskFaqCategory $helpdeskFaqCategoryRecord creates new record on each access!
 * @property-read HelpdeskCategory $helpdeskCategoryRecord creates new record on each access!
 ...
 *
 */
class Am_Di extends sfServiceContainerBuilder implements ContainerInterface
{
    static $instance;
    ...
    function init()
    {
        ...
        $this->register('pdoDb', '\Am\PdoDb')
            ->setConstructor('createFromDb')
        ...
        $this->register('rateLimit', 'Am_RateLimit')
            ->addArgument(new sfServiceReference('service_container'));
    ...
    public function getCacheBackendService()
    {
        if (!isset($this->services['cacheBackend']))
        {
            ...
            if (AM_HUGE_DB && !AM_HUGE_DB_NO_CACHE)
            {
    ...
    function getThemeService()
    {
        ...
        {
            $theme = $this->config->get('theme', 'default');
            $c = 'Am_Theme_' . ucfirst(toCamelCase($theme));
            if (file_exists($fn = AM_THEMES_PATH . '/' . $theme . '/Theme.php'))
                include_once $fn;
            $class = class_exists($c , true )? $c : 'Am_Theme';
    ...
    public function createMailTransport()
    {
        ...
        switch ($em_ = $this->config->get('email_method'))
        {
            case 'disabled':
                return new Am_Mail_Transport_Null;
            case 'smtp':
            ...
            case 'ses':
                $config = [
                    ...
                    'configurationSet' => $this->config->get('ses_configuration_set'),
                ];
            ...
            case 'gmailoauth':
                $config = [
                    'client_id' => $this->config->get('gmailoauth_client_id'),
                    'client_secret' => $this->config->get('gmailoauth_client_secret'),
                    'refresh_token' => $this->config->get('gmailoauth_refresh_token'),
                ];
                return new Am_Mail_Transport_GmailOauth($config);
            default:
                $emailing_methods = [];
                ...
                if(count($emailing_methods) && isset($emailing_methods[$em_])
                    && class_exists($cl = 'Am_Mail_Transport_' . ucfirst($em_)))
                {
                    return new $cl();
                }
                else {

...
class Am_Form_Element_OptionsEditor extends HTML_QuickForm2_Element_Input
{
    ...
    public function getRawValue()
    {
        ...
        return $this->doOrder($value ? json_decode($value, true) : false);
    }
...
class Am_Form_Element_Category extends HTML_QuickForm2_Container_Group
{
    public function __construct($name = null, $attributes = null, $data = null)
    {
        ...
        $rel = 'categories-' . $data['base_url'];

        $sel = $this->addSelect($name, [
            'multiple' => 'multiple',
            'class' => 'magicselect am-combobox-fixed',
            'rel' => $rel,
        ])
        ...
        $this->addHtml()->setHtml(
            sprintf(
                '<div><a class="local am-edit-category" id="edit-%s-link" href="%s" target="_top" title="%s" data-url="%s" data-rel="%s">%s</a></div>',
                $id,
                Am_Di::getInstance()->url($data['base_url']),
                Am_Html::escape($data['title']),
                Am_Di::getInstance()->url("{$data['base_url']}/options"),
                Am_Html::escape($rel),
                Am_Html::escape($data['link_title'])
            ));
    }
}
...
class Am_Form_Element_Upload extends HTML_QuickForm2_Element_InputText
{
    ...
    public function  __construct($name = null, $attributes = null, $data = null)
    {
        ...
        $this->urlGet = $data['urlGet'] ?? 'admin-upload/get';
    ...
    public function getFrozenHtml()
    {
        ...
        foreach (array_filter($value) as $upload_id) {
            ...
            $renderedFiles[] = sprintf(
                ...
                Am_Di::getInstance()->url($this->urlGet, ['id'=>$upload->pk()]),
                $upload->getName(),

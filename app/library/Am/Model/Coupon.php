...
class Coupon extends Am_Record_WithData
{
    ...
    protected function _isApplicable($item, $isFirstPayment = true)
    {
        ...
        if (!is_null($product_ids) || $bp_id) {
            ...
            if (
                !in_array($item->item_id, (array)$product_ids)
    ...
    /**
     * Check if require_product prevent_if_product coupon batch settings are satisfied
...
class CouponTable extends Am_Table_WithData
{
    ...
    function validateCode($code): bool
    {
        if (
            $this->getDi()->config->get('coupon_no_special_chars')
            && !preg_match("/^[0-9a-zA-Z]+$/", $code)
        ) {
            return false;
        }
        $couponMinLength = $this->getDi()->config->get('coupon_min_length',2);
        $couponMaxLength = $this->getDi()->config->get('coupon_max_length',16);

        if (
            strlen($code)>$couponMaxLength
            || strlen($code)<$couponMinLength
        ){
            return false;
        }

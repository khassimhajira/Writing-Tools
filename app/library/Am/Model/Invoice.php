...
class Invoice extends Am_Record_WithData
{
    ...
    public function approve()
    {
        ...
        foreach ($saved as $time => $v) {
            $this->addAccessPeriod($v[1], $v[0] ?: null);
    ...
    function calculate()
    {
        ...
        if (
            ...
            if (
                ((float)$bp->first_price == (float)$this->first_total)
        ...
        $this->terms = $this->getDi()->hook->filter($this->terms, Am_Event::INVOICE_TERMS, ['invoice' => $this]);
     ...
     * @todo actual calculations in Invoice::isFirstPayment
     * @return boolean
     */
...
//                    return $this->_getItemCompatibleError('SECONDPERIOD', $item, $existingItem);
    ...
    function insert($reload = true)
    {
        ...
        for ($i = 0; $i <= $maxAttempts; $i++)
            try {
                ...
                if ($e->getTable() != '?_invoice' || $i >= $maxAttempts)
                    throw $e;
                $this->public_id = null;
            }
        foreach ($this->_items as $item)
    ...
    function delete()
    {
        ...
        $this->getDi()->invoicePaymentTable->deleteByInvoiceId($this->pk());
        $this->getDi()->invoiceRefundTable->deleteByInvoiceId($this->pk());
    ...
    public function addAccessPeriod(Am_Paysystem_Transaction_Interface $transaction, $invoicePaymentId = null)
    {
        ...
        if ($isFirstPeriod) {
            ...
            $this->getDi()->hook->call(new Am_Event_InvoiceStarted(null, [
                ...
                'payment' => $invoicePaymentId ? $this->getDi()->invoicePaymentTable->load($invoicePaymentId, false) : null
            ]));
        } else {
            $this->updateStatus();
            $this->getUser()->checkSubscriptions(true);
     ...
     *
     * @param bool $isFirst period that will be added;
     * @param string $date  - date that will be used instead of current rebill_date setting;
     *
     */
    function addToRebillDate(bool $isFirst, $date=null)
    {
...
//                        $a->updateQuick('expire_date', $yesterday->format('Y-m-d'));
        ...
        $refundAmount = abs(floatval($refundAmount));
    ...
    public function stopAccess(Am_Paysystem_Transaction_Interface $transaction)
    {
        ...
        if (
            $this->second_period == Am_Period::MAX_SQL_DATE
    ...
    {
        return $this->_db->selectCell("SELECT MAX(expire_date) FROM ?_access WHERE invoice_id=?d", $this->invoice_id);
    }
    ...
    {
        return $this->_db->selectCell("SELECT MAX(dattm) FROM ?_invoice_payment WHERE invoice_id=?d", $this->invoice_id);
    }
    ...
    public function setCancelled($cancelled = true)
    {
        ...
        if (!$cancelled) {
            ...
            $this->getDi()->hook->call(Am_Event::INVOICE_AFTER_RESTART, ['invoice' => $this]);
        }
        if ($cancelled) {
    ...
    {
        return $this->terms ?: $this->getTermsText();
    }
    ...
    {
        if (!$this->getDi()->hook->filter($this->_canUpgrade($item, $upgrade), Am_Event::CAN_UPGRADE, [
            'item' => $item,
            'upgrade' => $upgrade
        ])) {
            return false;
    ...
    protected function _canUpgrade(InvoiceItem $item, ProductUpgrade $upgrade)
    {
        $expire_date = $this->getDi()->db->selectCell("SELECT MAX(expire_date) FROM ?_access WHERE invoice_item_id=?", $item->pk());
        if ($expire_date < $this->getDi()->sqlDate) {
            return false;
        }
        ...
        if ($item->billing_plan_id != $upgrade->from_billing_plan_id)
            return false;
        if ($upgrade->only_recurring && $this->status != Invoice::RECURRING_ACTIVE) {
            return false;
        }
        ...
        foreach ($this->getItems() as $it) {
            if ($item->invoice_item_id != $it->invoice_item_id) {
                if ($it->item_type == 'surcharge') {
                    $toSkip[] = $it;
                }
                if ((float)$it->second_total && $it->item_type != 'surcharge') {
                    return false;
        ...
        $error = $this->isItemCompatible($newItem, $toSkip);
        if (null != $error)
            return false;
    ...
    function doRestoreRecurring()
    {
        ...
        foreach ($this->getItems() as $item)
        {
            ...
            // Do not include items if restore for it is not allowed;
            if (!in_array($item->item_id, $allowRestorePids)) continue;
        ...
        $newInvoice->due_date = sqlDate('today');
    ...
    function recalculateWithTaxRate($rate): Invoice
    {
        ...
        $this->getDi()->hook->add(Am_Event::INVOICE_GET_CALCULATORS, function (Am_Event $event) use ($rate) {
    ...
    function findForRebill($date, $paysys_id = null)
    {
        return $this->selectObjects("
            SELECT * FROM ?_invoice
                WHERE rebill_date = ?
                    AND IFNULL(tm_cancelled,0)=0
                    AND status=?
                    { AND paysys_id = ? }",
            $date,
            Invoice::RECURRING_ACTIVE,
            $paysys_id ?: DBSIMPLE_SKIP
        );
    }

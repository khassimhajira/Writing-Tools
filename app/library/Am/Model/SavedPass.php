...
class SavedPassTable extends Am_Table
{
    ...
    const PASSWORD_WORDPRESS_BCRYPT = 'password_wordpress_bcrypt';
    ...
    function getPasswordFormats()
    {
        $types = [
            ...
            self::PASSWORD_WORDPRESS_BCRYPT => null
        ];
    ...
    {
        switch ($methodConst) {
            // accept all wordpress passwords here
            case self::PASSWORD_WORDPRESS_BCRYPT:
                if($salt && User::verifyPassword($pass, $salt))
                    return $salt;
                $password_to_hash = base64_encode( hash_hmac( 'sha384', trim( $pass ), 'wp-sha384', true ) );

...
class SavedForm extends Am_Record_WithData
{
    ...
    public function addDefaultFor($d): self
    {
    ...
    public function delDefaultFor($d): self
    {
        $def = $this->getDefaultFor();
    ...
    function addBrickConfig($row): self
    {
    ...
    function removeBrickConfig($class, $id = null): self
    {
        $fields = $this->getFields();
        $count = 0;
...
class SavedFormTable extends Am_Table_WithData
{
    ...
    public function getOptions($type = null)
    {
        return $this->_db->selectCol(
            "SELECT saved_form_id AS ARRAY_KEY, CONCAT(title, ' (', comment, ')') FROM {$this->_table} {WHERE type=?} ORDER BY title",
            $type ?: DBSIMPLE_SKIP
        );
    }
    ...
    function addTypeDef(array $typeDef): self
    {
    ...
    protected function runEventOnce()
    {
        if (!$this->_eventCalled) {
            ...
            $this->getDi()->hook->call(Am_Event::SAVED_FORM_TYPES, ['table' => $this]);
        }
    }

...
class Product extends Am_Record_WithData implements IProduct
{
    ...
    function setCategories(array $categories)
    {
        $this->getAdapter()->query(
            "DELETE FROM ?_product_product_category WHERE product_id=?d {AND product_category_id NOT IN (?a)}",
            $this->product_id,
            $categories ?: DBSIMPLE_SKIP
        );
...
class ProductTable extends Am_Table_WithData
{
    ...
    function getVisible($productCategories = null, $activeProductIds = null, $expiredProductIds = null)
    {
        ...
        return $this->selectObjects("SELECT p.*
            ...
            ",  $productCategories ?: DBSIMPLE_SKIP);
    }
    ...
    function getProductTitles(array $ids)
    {
        return ($ids = array_filter(array_map('intval', $ids)))
            ? array_map(
                ['Product', 'stripEscape'],
                $this->_db->selectCol("SELECT product_id AS ARRAY_KEY, title FROM ?_product WHERE product_id IN (?a)", $ids)
            )
            : [];
    }

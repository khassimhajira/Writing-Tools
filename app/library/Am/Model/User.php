 ...
 * @property datetime $mobile_confirmation_date
...
class User extends Am_Record_WithData
{
    ...
    {
        return in_array($name, ['unsubscribe_link', 'mobile']);
    }
    ...
    function delete()
    {
        ...
        if ($this->getDi()->invoiceTable->countBy(['user_id' => $this->pk(), 'status' => Invoice::RECURRING_ACTIVE])) {
            throw new Am_Exception_User_Has_Recurring("User #{$this->pk()}({$this->login}) has active recurring subscriptions. It is necessary to cancel all recurring subscription before delete user account.");
        }
    ...
    function update()
    {
        ...
        $oldU->user_id = null;
        if (
            $this->getDi()->hook->have(['userBeforeUpdate', 'userAfterUpdate', 'subscriptionUpdated'])
    ...
    function checkPassword($pass)
    {
        ...
        return self::verifyPassword($pass, $this->pass);
    }

    static function verifyPassword($pass, $hashedPass)
    {
        if (str_starts_with($hashedPass, '$wp')) {
            $password_to_verify = base64_encode(hash_hmac('sha384', $pass, 'wp-sha384', true));
            $check = password_verify($password_to_verify, substr($hashedPass, 3));
        } elseif (str_starts_with($hashedPass, '$P$')) {
            $check = (new PasswordHash(8, true))->CheckPassword($pass, $hashedPass);
        } else {
            $check = password_verify($pass, $hashedPass);
        }
        return $check;
    }
     ...
     */
    function setPass(string $pass, bool $quick = false): self
    {
    ...
    static function cryptPass(string $pass, bool $quick = false): string
    {
     ...
     */
    function getStoredPlaintextPassword(): ?string
    {
    ...
    function getLoginCookie(): string
    {
    ...
    function checkSubscriptions(bool $updateCache = false)
    {
        if (!$this->user_id)
    ...
    {
        if (
            $this->getDi()->config->get('changepass_mail')
            && ($et = Am_Mail_Template::load('changepass_mail', $this->lang))
        ) {
            $et->setUser($this);
            $et->setPassword($this->getPlaintextPass());
            $et->send($this);
    ...
    /**
     * Return true if user have any active subscriptions
     */
    ...
    function isPaid(): bool
    {
        return (bool)$this->getAdapter()->selectCell(
            "SELECT SUM(amount)>0 FROM ?_invoice_payment p WHERE user_id=?d",
            $this->user_id
        );
    }
    ...
    function getExpire($productIdOrIds = [])
    {
        ...
        return $this->getDi()->db->selectCell("SELECT MAX(expire_date)
            FROM ?_access
            WHERE user_id=?d { AND product_id IN (?a) }", $this->pk(), $productIdOrIds ?: DBSIMPLE_SKIP
        );
    }
    ...
    function getBegin($productIdOrIds = [])
    {
        ...
        return $this->getDi()->db->selectCell("SELECT MIN(begin_date)
            FROM ?_access
            WHERE user_id=?d AND begin_date > ? { AND product_id IN (?a) }", $this->pk(), sqlDate('now'), $productIdOrIds ?: DBSIMPLE_SKIP
        );
    }
    ...
    function getActiveCategoriesExpiration()
    {
        ...
        $out = array_map(function ($pids) {
            return $this->getExpire($pids);
        }, $cp_map);
    ...
    function getActiveProductIds()
    {
        return $this->getAdapter()->selectCol(
            "SELECT product_id FROM ?_user_status WHERE user_id=?d AND status=?d",
            $this->user_id,
            self::STATUS_ACTIVE
        );
    }
    ...
    function getGroups()
    {
        ...
        return $this->getAdapter()->selectCol(
            "SELECT DISTINCT user_group_id FROM ?_user_user_group WHERE user_id=?d",
            $this->user_id
        );
    }
    ...
    {
        $period = $this->getDi()->config->get('force_change_password_period', 30);
        return $this->data()->get('force_change_password')
            || (
                $this->getDi()->config->get('force_change_password')
                && $this->pass_dattm < sqlTime("-$period day")
            );
    }
...
class UserTable extends Am_Table_WithData
{
     ...
     * Return True if record unique (no such login exists), false if not-unique
     */
    function checkUniqLogin(string $login, $user_id = null)
    {
        $this->getDi()->hook->call(Am_Event::PRE_CHECK_UNIQ_LOGIN, ['login' => $login, 'userId' => $user_id]);

        $u = $this->_db->selectCell(
            "SELECT user_id FROM ?_user WHERE login=? { AND user_id <> ?d }",
            $login,
            $user_id ?: DBSIMPLE_SKIP
        );
        if ($u)
            return 0;
     ...
     * Return True if record unique (no such login exists), false if not-unique
     */
    function checkUniqEmail(string $email, $user_id = null)
    {
        $this->getDi()->hook->call(Am_Event::PRE_CHECK_UNIQ_EMAIL, ['email' => $email, 'userId' => $user_id]);

        $u = $this->_db->selectCell(
            "SELECT user_id FROM ?_user WHERE email=? { AND user_id <> ?d }",
            $email,
            $user_id ?: DBSIMPLE_SKIP
        );
        if ($u)
    ...
    function checkAllSubscriptions()
    {
        ...
        while ($changed = $this->checkAllSubscriptionsFindChanged(1000)) {
            foreach ($changed as $user_id) {
                ...
                if ($u)
                    $u->checkSubscriptions(false); // checked above
            }
        }
    }

    /**
     * Find user record by email (if $login looks like an email) or by username
     */
    function getByLoginOrEmail(string $login): ?User
    {
        if (!strlen($login))
            return null;
     ...
     */
    function getAuthenticatedRow(string $login, string $pass, &$code = null): ?User
    {
    ...
    function getAuthenticatedCookieRow(?string $cLogin, ?string $cPass, &$code = null): ?User
    {
    ...
    {
        return $this->selectObjects("SELECT m.user_id, m.name_f, m.name_l, m.login, m.email, m.status, m.added, m.remote_addr,
    ...
    function getPersonalDataFieldOptions()
    {
        $pdFields = [
            ...
            'country' =>  ___('Country'),
            'phone' => ___('Phone'),
            'mobile_area_code' => ___('Mobile Area Code'),

...
class ProductOption extends Am_Record
{
     ...
     * get key-value options for select
     */
    function getSelectOptionsWithPrice(BillingPlan $plan, $isHtml = true)
    {
        $o = $this->getDecodedOptions();
        if (empty($o['options'])) return [];
        $ret = [];
        ...
        foreach ($o['options'] as $v)
        {
            ...
            {
                $ob->first_price = empty($prices[0]) ? 0 : abs(floatval($prices[0]));
                $ob->second_price = empty($prices[1]) ? 0 : abs(floatval($prices[1]));
                $t = new Am_TermsText($ob);
                $sign = $prices[0] > 0 ? '+' : '&minus;';
                $format = $isHtml ? ' <span class="am-option-price">%s</span>' : ' (%s)';
                $priceLabel = sprintf($format, $sign . $t->getStringForOption());
            }
